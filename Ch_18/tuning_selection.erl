...
dynamic(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	...
	ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,AgeLimit,PerturbationRange,AnnealingParameter,[]) of
		[] ->
			[N_Id|_] = N_Ids,
...
%The dynamic/4 selection function randomly selects an age limit for its neuron id pool. The age limit is chosen by executing math:sqrt(1/rand:uniform()), which creates a value between 1 and infinity. Using this function there is 75% that the number will be =<2, 25% that it will be >=2, 11% that it will be >=3... Everytime this selection function is executed, the AgeLimit is generated anew, thus different times it will produce different neuron id pools for tuning.
	...
	extract_CurGenNeuronIDPs([N_Id|N_Ids],Generation,AgeLimit,PR,AP,Acc)->
		...
		case NeuronGen >= (Generation-AgeLimit) of
			true ->
				...
				extract_CurGenNeuronIDPs(N_Ids,Generation,AgeLimit,PR,AP,[{N_Id,Spread}|Acc]);
			false ->
				extract_CurGenNeuronIDPs(N_Ids,Generation,AgeLimit,PR,AP,Acc)
		end;
...
dynamic_random(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter) ->
			ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,math:sqrt(1/rand:uniform()),PerturbationRange,AnnealingParameter,[]) of
				[] ->
					[N_Id|_] = N_Ids,
					[{N_Id,PerturbationRange*math:pi()}];
				ExtractedNWeightedInputs->
...
active(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,3,PerturbationRange,AnnealingParameter,[]).
...
active_random(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,3,PerturbationRange,AnnealingParameter,[]) of
		[] ->
			[N_Id|_] = N_Ids,
			[{N_Id,PerturbationRange*math:pi()}];
		ExtractedNWeightedInputs->
...
current(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	case extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,0,PerturbationRange,AnnealingParameter,[]) of
		[] ->
			[N_Id|_] = N_Ids,
			[{N_Id,PerturbationRange*math:pi()}];
		IdPs ->
...
current_random(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,0,PerturbationRange,AnnealingParameter,[]) of
		[] ->
			[N_Id|_] = N_Ids,
			[{N_Id,PerturbationRange*math:pi()}];
		IdPs ->
...
all(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,AgentGeneration,PerturbationRange,AnnealingParameter,[]).
...
all_random(N_Ids,AgentGeneration,PerturbationRange,AnnealingParameter)->
	ChosenNWeightedInputs = extract_CurGenNeuronIDPs(N_Ids,AgentGeneration,AgentGeneration,PerturbationRange,AnnealingParameter,[]),
	Total_Neurons = length(ChosenNWeightedInputs),
	MutationP = 1/math:sqrt(Total_Neurons),
	choose_randomNeuronIDPs(MutationP,ChosenNWeightedInputs).
