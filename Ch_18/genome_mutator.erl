...
mutate(Agent_Id)->
	...
	F = fun()->
		...
		TotalMutations = tot_topological_mutations:TTM_Name(Parameter,Agent_Id),
		OldGeneration = A#agent.generation,
		NewGeneration = OldGeneration+1,
		genotype:write(A#agent{generation = NewGeneration}),
		apply_Mutators(Agent_Id,TotalMutations),
	...
	mutate_SearchParameters(Agent_Id)->
		case rand:uniform() < ?SEARCH_PARAMTERS_MUTATION_PROBABILITY of
			true ->
				...
				apply_ESMutators(Agent_Id,TotalMutations);
			false ->
				ok
		end.
...
%The mutate_weights/1 function accepts the Agent_Id parameter, extracts the NN's cortex, and then chooses a random neuron belonging to the NN with a uniform distribution probability. Then the neuron's weighted_inputs list is extracted, and the function perturbWeightedInputs/1 is used to perturb/mutate the weights. Once the WeightedInputs have been perturbed, the agent's evolutionary history, EvoHist is updated to include the successfully applied mutate_weights mutation operator. Then the updated Agent and the updated neuron are written to the database.
	...
	perturbWeightedInputs(WeightedInputs)->
		Total_WeightsP=lists:sum([length(WeightsP) || {_Input_Id,WeightsP}<-WeightedInputs]),
		MP = 1/math:sqrt(Total_WeightsP),
		perturbWeightedInputs(MP,WeightedInputs,[]).
	perturbWeightedInputs(MP,[{Input_Id,WeightsP}|WeightedInputs],Acc)->
		U_WeightsP = perturb_weightsP(MP,WeightsP,[]),
		perturbWeightedInputs(MP,WeightedInputs,[{Input_Id,U_WeightsP}|Acc]);
...
mutate_pf(Agent_Id)->
	...
	case (A#agent.constraint)#constraint.neural_pfns -- [PFName] of
		...
		Other_PFNames ->
			...
			UWeightedInput = [{Input_IdP,plasticity:New_PFName(weight_parameters)} || {Input_IdP,_OldPL} <- InputIdPs],
...
mutate_plasticity_parameters(Agent_Id)->
	...
	UpdatedNeuron = plasticity:PFName({N_Id,mutate}),
	EvoHist = A#agent.evo_hist,
...
%link_FromNeuronToNeuron/3 establishes a link from neuron with id From_NeuronId, to a neuron with id To_NeuronId. The function then calls link_FromNeuron/4, which establishes the link on the From_NeuronId's side. The updated neuron associated with the From_NeuronId is then written to database. To decide how long the weight list that is going to be added to the To_NeuronId's weighted_inputs, the function calculates From_NeuronId's output vector length. Since the connection is from a neuron, FromOVL is set to 1. link_ToNeuron/4 is then called, and the link is established on the To_NeuronId's side. Finally, the updated neuron associated with the id To_NeuronId is written to database. The order of reading the FromN and ToN neuron records from the database is important. It is essential that ToN is read after the U_FromN is written to database, in the case that From_NeuronId and To_NeuronId refer to the same neuron (a recurrent connection from the neuron to itself). If both neurons are read at the same time for example before the links are established, then the link established in the U_FromN will be overwritten when the U_ToN is written to file. Thus order is important in this function.
	...
	link_FromNeuron(FromN,ToId,Generation)->
		...
		case lists:member(ToId, FromOutputIDs) of
			...
			false ->
				{U_FromOutputIDs,U_FromRO_Ids} = case FromLI >= ToLI of
					true ->
						{[ToId|FromOutputIDs],[ToId|FromRO_Ids]};
					false ->
						{[ToId|FromOutputIDs],FromRO_Ids}
				end,
				FromN#neuron{
...
%To Part
	...
	case length(Fanin_Ids) >= ToA#actuator.vector_length of
		true ->
...
%The function Link_FromNeuronToActuator/4 establishes a link eminating from the neuron with an id From_NeuronId, to an actuator with the id To_ActuatorId. First the From_NeuronId's record is updated using the function link_FromNeuron/3, after which the updated neuron record is written to database. Then the function checks whether the actuator to which the neuron is establishing the link, still has space for that link (length(Fanin_Ids) is less than the actuator's vector length, vector_length). If there is no more room, then the function exits with error, if there is room, then the actuator's fanin_ids list is updated by appending to it the id of the neuron's id. The updated actuator is then written to the database.
...
add_outlink(Agent_Id)->
	...
	Outlink_NeuronIDPool = filter_OutlinkIdPool(A#agent.constraint,N_Id,N_Ids),
...
add_inlink(Agent_Id)->
	...
	N_Id = lists:nth(rand:uniform(length(N_Ids)),N_Ids),
	Neuron = genotype:read({neuron,N_Id}),
	{I_Ids,_WeightPLists} = lists:unzip(N#neuron.weighted_inputs),
	Inlink_NeuronIDPool = filter_InlinkIdPool(A#agent.constraint,N_Id,N_Ids),
	case lists:append(S_Ids,Inlink_NeuronIDPool) -- I_Ids of
		[] ->
...
add_neuron(Agent_Id)->%Adds neuron and connects it to other neurons, not sensors or actuators.
	...
	genotype:construct_Neuron(CortexID,Generation,SpecCon,NewN_Id,[],[]),
	Inlink_NeuronIDPool = filter_InlinkIdPool(A#agent.constraint,NewN_Id,N_Ids),
	Outlink_NeuronIDPool = filter_OutlinkIdPool(A#agent.constraint,NewN_Id,N_Ids),
	FromElementId_Pool = Inlink_NeuronIDPool++S_Ids,
	ToElementId_Pool = Outlink_NeuronIDPool,
	case (FromElementId_Pool == []) or (ToElementId_Pool == []) of
		true ->
			exit("********ERROR::add_neuron(Agent_Id)::Can't add new neuron here, Inlink_NeuronIDPool or Outlink_NeuronIDPool is empty.");
		false ->
			...
			genotype:write(Cortex#cortex{neuron_ids = U_N_Ids}),
...
%Updated agent
	...
	U_Cortex = Cortex#cortex{neuron_ids = [NewN_Id|Cortex#cortex.neuron_ids]},
...
get_NewLI(FromLI,ToLI,Direction,Pattern)->
	...
	NewLI.
%get_NewLI/4 calculates or creats a new layer index located between FromLI and ToLI. The function calls get_NextLI/3 or get_PrevLI/3, depending on whether the direction of the connection is forward, from sensors towards actuators (Direction = next) or from actuators towards sensors (Direction = prev), which is the case when executing an insplice/1 function, which calculates or creates a new layer between the N_Id and one of the ids in its weighted_inputs list. If the FromLI == ToLI, the function exits with an error.
...
add_sensor(Agent_Id)->%TODO: There should be a preference towards adding sensors not yet used.
	...
	CortexID = Agent#agent.cortex_id,
	Cortex = genotype:read({cortex,CortexID}),
	S_Ids = Cortex#cortex.sensor_ids,
	SpeCon = Agent#agent.constraint,
	Morphology = SpeCon#constraint.morphology,
	case morphology:get_Sensors(Morphology)--[(genotype:read({sensor,S_Id}))#sensor{id=undefined,cortex_id=undefined,fanout_ids=[],generation=undefined} || S_Id<-S_Ids] of
		[] ->
		...
		Available_Sensors ->
			...
			NewSensor=(lists:nth(rand:uniform(length(Available_Sensors)),Available_Sensors))#sensor{id=NewS_Id,cortex_id=CortexID},
			EvoHist = Agent#agent.evo_hist,
			...
			U_Cortex = Cortex#cortex{sensor_ids=[NewS_Id|S_Ids]},
			genotype:write(U_Cortex),
			genotype:write(Agent#agent{evo_hist=U_EvoHist})
	end.
...
add_actuator(Agent_Id)->%TODO: There should be a preference towards adding actuators not yet used.
	...
	CortexID = Agent#agent.cortex_id,
	Cortex = genotype:read({cortex,CortexID}),
	A_Ids = Cortex#cortex.actuator_ids,%TODO: Should we fill in all the fanin_ids locations, or just 1? and let evolution fill the rest?
	SpeCon = Agent#agent.constraint,
	Morphology = SpeCon#constraint.morphology,
	case morphology:get_Actuators(Morphology)--[(genotype:read({actuator,A_Id}))#actuator{cortex_id=undefined,id=undefined,fanin_ids=[],generation=undefined} || A_Id<-A_Ids] of
		[] ->
			exit("********ERROR:add_actuator(Agent_Id):: NN system is already using all available actuators");
		Available_Actuators ->
			io:format("Used actuators:~p~n Filtered avaialble actuators:~p~n",[morphology:get_Actuators(Morphology),[(genotype:read({actuator,A_Id}))#actuator{cortex_id=undefined,id=undefined,fanin_ids=[],generation=undefined} || A_Id<-A_Ids]]),
			io:format("Available_Actutors:~p~n",[Available_Actuators]),
			NewA_Id = {{1,genotype:generate_UniqueId()},actuator},
			NewActuator=(lists:nth(rand:uniform(length(Available_Actuators)),Available_Actuators))#actuator{id=NewA_Id,cortex_id=CortexID},
			EvoHist = Agent#agent.evo_hist,
			case Agent#agent.encoding_type of
				neural ->
			...
			U_Cortex = Cortex#cortex{actuator_ids=[NewA_Id|A_Ids]},
			genotype:write(U_Cortex),
			genotype:write(Agent#agent{evo_hist=U_EvoHist})
	end.
...
add_cpp(Agent_Id)->%TODO: There should be a preference towards adding substrate_cpps not yet used.
	...
	case Agent#agent.encoding_type of
		...
		substrate->
			...
			case morphology:get_SubstrateCPPs(Dimensions,Plasticity)--[(genotype:read({sensor,CPP_Id}))#sensor{id=undefined,cortex_id=undefined,fanout_ids=[],generation=undefined} || CPP_Id<-CPP_Ids] of
				[] ->
				...
				Available_CPPs ->
					...
					NewCPP=(lists:nth(rand:uniform(length(Available_CPPs)),Available_CPPs))#sensor{id=NewCPP_Id,cortex_id=CortexID},
					EvoHist = Agent#agent.evo_hist,
...
add_cep(Agent_Id)->
	...
	case Agent#agent.encoding_type of
		...
		substrate->
			...
			case morphology:get_SubstrateCEPs(Dimensions,Plasticity)--[(genotype:read({actuator,CEP_Id}))#actuator{id=undefined,cortex_id=undefined,fanin_ids=[],generation=undefined} || CEP_Id<-CEP_Ids] of
				[] ->
				...
				Available_CEPs ->
					...
					NewCEP=(lists:nth(rand:uniform(length(Available_CEPs)),Available_CEPs))#actuator{id=NewCEP_Id,cortex_id=CortexID},
					EvoHist = Agent#agent.evo_hist,
