...
discrete_tmaze(sensors)->
	[
		#sensor{name=dtm_GetInput,type=standard,scape={private,dtm_sim},vector_length=7,parameters=[all]}
	];
...
prey(actuators)->
	Movement = [#actuator{name=two_wheels,type=standard,scape={public,flatland},format=no_geo,vector_length=2,parameters=[2]}],
	Movement;
prey(sensors)->
	Pi = math:pi(),
	Distance_Scanners = [#sensor{name=distance_scanner,type=standard,scape={public,flatland},format=no_geo,vector_length=Density,parameters=[Spread,Density,ROffset]} || 
		Spread<-[Pi/2],Density<-[5], ROffset<-[Pi*0/2]],
	Color_Scanners = [#sensor{name=color_scanner,type=standard,scape={public,flatland},format=no_geo,vector_length=Density,parameters=[Spread,Density,ROffset]} ||
		Spread <-[Pi/2], Density <-[5], ROffset<-[Pi*0/2]],
	Energy_Scanners = [#sensor{name=energy_scanner,type=standard,scape={public,flatland},format=no_geo,vector_length=Density,parameters=[Spread,Density,ROffset]} ||
		Spread <-[Pi/2], Density <-[5], ROffset<-[Pi*0/2]],
	Stat_Readers = [#sensor{name=Name,type=standard,scape={public,flatland},format=no_geo,vector_length=Density,parameters=Param} || {Name,Density,Param} <- [{energy_reader,1,[]}]],
	Distance_Scanners.
