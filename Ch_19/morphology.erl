...
get_SubstrateCEPs(Dimensions,Plasticity)->
	case Plasticity of
		...
		abcn ->
			[#actuator{name=set_abcn,type=substrate,vector_length=5}]; %[{abcn,4}];
		none ->
...
discrete_tmaze(sensors)->
	[
		#sensor{name=dtm_GetInput,type=standard,scape={private,dtm_sim},vector_length=4,parameters=[all]}
	];
...
prey(actuators)->
	Movement = [#actuator{name=differential_drive,type=standard,scape={public,flatland}, vector_length=2, parameters=[2]}],
	Movement;
prey(sensors)->
	Pi = math:pi(),
	Color_Scanners = [#sensor{name=color_scanner,type=standard,scape={public,flatland},vector_length=Density, parameters=[Spread,Density,ROffset]} || Spread <-[Pi/2], Density <-[5], ROffset<-[Pi*0/2]],
	Range_Scanners = [#sensor{name=range_scanner,type=standard,scape={public,flatland},vector_length=Density, parameters=[Spread,Density,ROffset]} || Spread <-[Pi/2], Density <-[5], ROffset<-[Pi*0/2]],
	Color_Scanners++Range_Scanners.
	
forex_trader(actuators)->
	[
		#actuator{name=fx_Trade,type=standard,scape={private,fx_sim},format=no_geo,vector_length=1,parameters=[]}
	];
forex_trader(sensors)->
	PLI_Sensors=[#sensor{name=fx_PLI,type=standard,scape={private,fx_sim},format=no_geo,vector_length=HRes,parameters=[HRes,close]} || HRes<-[10]],
	PCI_Sensors = [#sensor{name=fx_PCI,type=standard,scape={private,fx_sim},format={symmetric,[HRes,VRes]},vector_length=HRes*VRes,parameters=[HRes,VRes]} || HRes <-[50], VRes<-[20]],
	InternalSensors = [#sensor{name=fx_Internals,type=standard,scape={private,fx_sim},format=no_geo,vector_length=3,parameters=[3]}],%[Long|Short|Void],Value
