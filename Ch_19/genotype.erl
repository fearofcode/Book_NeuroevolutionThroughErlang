...
constructAgent(SpecieID,AgentID,SpecCon)->
	...
	{CortexID,Pattern,SubstrateID} = constructCortex(AgentID,Generation,SpecCon,Encoding_Type,SPlasticity,SLinkform),
	Agent = #agent{
		id = AgentID,
...
constructCortex(AgentID,Generation,SpecCon,Encoding_Type,SPlasticity,SLinkform)->
	...
	case Encoding_Type of
		...
		substrate ->
			SubstrateID={{void,generate_UniqueId()},substrate},
			Sensors = [S#sensor{id={{-1,generate_UniqueId()},sensor},cortex_id=CortexID,generation=Generation,fanout_ids=[SubstrateID]}|| S<- morphology:get_InitSensors(Morphology)],
			Actuators = [A#actuator{id={{1,generate_UniqueId()},actuator},cortex_id=CortexID,generation=Generation,fanin_ids=[SubstrateID]}||A<-morphology:get_InitActuators(Morphology)],
			[write(S) || S <- Sensors],
			[write(A) || A <- Actuators],
			...
			NeuronIDs=constructInitialNeuroLayer(CortexID,Generation,SpecCon,Substrate_CPPs,Substrate_CEPs,[],[]),
			%io:format("Sensors:~p~n Actuators:~p~n Substate_CPPs:~p~n Substrate_CEPs:~p~n",[Sensors,Actuators,Substrate_CPPs,Substrate_CEPs]),
			SensorIDs = [S#sensor.id || S<-Sensors],
			ActuatorIDs = [A#actuator.id || A<-Actuators],
			CPPIDs = [CPP#sensor.id || CPP<-Substrate_CPPs],
			CEPIDs = [CEP#actuator.id || CEP<-Substrate_CEPs],
			Substrate = #substrate{
				id = SubstrateID,
				agent_id = AgentID,
				cpp_ids = CPPIDs,
	...
	write(Cortex),
	{CortexID,[{0,NeuronIDs}],SubstrateID}.
...
%constructInitialNeurons/6 accepts the list of sensors and a single actuator, connects each neuron to the actuator, and randomly chooses whether to connect it from all the sensors or a subset of the given sensors. Once all the neurons have been connected to the actuator and from the sensors, the updated sensors, whose fanout_ids have been updated with the ids of the neurons, are returned to the caller.
		...
		construct_Neuron(CortexID,Generation,SpecCon,NeuronID,InputSpecs,OutputIDs)-> 
			...
			createWeightedInput(PF,[{InputID,InputVL}|WeightedInputs],Acc) ->
				WeightsP = createNeuralWeightsP(PF,InputVL,[]),
				createWeightedInput(PF,WeightedInputs,[{InputID,WeightsP}|Acc]); 
	...
	F = fun()->
		...
		case A#agent.substrate_id of
			undefined ->
				ok;
			SubstrateID->
				Substrate = read({substrate,SubstrateID}),
	...
	F = fun()->
		...
		case A#agent.substrate_id of
			...
			SubstrateID ->
				...
				[CloneSubstrateID] = map_ids(IdsNCloneIds,[A#agent.substrate_id],[]),
				CloneCPPIDs = map_ids(IdsNCloneIds,Substrate#substrate.cpp_ids,[]),
				CloneCEPIDs = map_ids(IdsNCloneIds,Substrate#substrate.cep_ids,[]),
				clone_neurons(IdsNCloneIds,Cortex#cortex.neuron_ids),
				clone_sensors(IdsNCloneIds,Cortex#cortex.sensor_ids),
				clone_actuators(IdsNCloneIds,Cortex#cortex.actuator_ids),
				Substrate = read({substrate,A#agent.substrate_id}),
...
%clone_actuators/2 accepts as input the name of the ets table and the list of actuator ids. It then goes through every actuator id, reads the actuator from the database, and updates all the ids (id, cortex_id, and fanin_ids) from their original values, to their clone values stored in the ets table. Then the new version of the actuator is written to the database.
	...
	clone_neurons(TableName,[NeuronID|NeuronIDs])->
		...
		CloneWeightedInputs =  [{ets:lookup_element(TableName,IID,2),WeightsP}|| {IID,WeightsP} <- N#neuron.weighted_inputs],
		CloneWeightedInputs_Modulation =  [{ets:lookup_element(TableName,IID,2),WeightsP}|| {IID,WeightsP} <- N#neuron.weighted_inputs_modulation],
...
create_test()->
	SpecieID = test,
	AgentID = test,
	SpecCon = #constraint{morphology=forex_trader,connection_architecture=feedforward, population_evo_alg_f=generational,neural_afs=[tanh],agent_encoding_types=[substrate],substrate_plasticities=[none]},
	F = fun()->
