...
prep(ExoSelfProcess) ->
	random:seed(now()),
	receive 
		{ExoSelfProcess,{Id,CortexProcess,ActivationFunction,PF,AggrF,WeightedInputProcess,OutputProcess,ROProcess}} ->
			fanout(ROProcess,{self(),forward,[?RO_SIGNAL]}),
			IProcessIDs = [IProcessID || {IProcessID,_W} <- WeightedInputProcess, IProcessID =/= bias],
			io:format("IWeightedInputProcessIDs:~p~n",[WeightedInputProcess]),
			loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{IProcessIDs,IProcessIDs},[],{WeightedInputProcess,WeightedInputProcess},OutputProcess,ROProcess)
	end.
...
loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{[InputProcess|IProcessIDs],MIProcessIDs},IAcc,{WeightedInputProcess,MWeightedInputProcess},OutputProcess,ROProcess)->
	receive
		{InputProcess,forward,Input}->
			loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{IProcessIDs,MIProcessIDs},[{InputProcess,Input}|IAcc],{WeightedInputProcess,MWeightedInputProcess},OutputProcess,ROProcess);
		{ExoSelfProcess,weight_backup}->
			loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{[InputProcess|IProcessIDs],MIProcessIDs},IAcc,{WeightedInputProcess,WeightedInputProcess},OutputProcess,ROProcess);
		{ExoSelfProcess,weight_restore}->
			loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{[InputProcess|IProcessIDs],MIProcessIDs},IAcc,{MWeightedInputProcess,MWeightedInputProcess},OutputProcess,ROProcess);
		{ExoSelfProcess,weight_perturb,Spread}->
			...
			loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{[InputProcess|IProcessIDs],MIProcessIDs},IAcc,{Perturbed_IWeightedInputProcessIDs,MWeightedInputProcess},OutputProcess,ROProcess);
		{ExoSelf,reset_prep}->
			neuron:flush_buffer(),
			ExoSelf ! {self(),ready},
			receive 
			...
			loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{MIProcessIDs,MIProcessIDs},[],{WeightedInputProcess,MWeightedInputProcess},OutputProcess,ROProcess);
...
loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{[],MIProcessIDs},IAcc,{WeightedInputProcess,MWeightedInputProcess},OutputProcess,ROProcess)->
	Ordered_IAcc = lists:reverse(IAcc),
	Aggregation_Product = signal_aggregator:AggrF(Ordered_IAcc,WeightedInputProcess),
	Output = functions:ActivationFunction(Aggregation_Product),
	U_IWeightedInputProcessIDs = plasticity:PF(Ordered_IAcc,WeightedInputProcess,Output),
	[OutputProcess ! {self(),forward,[Output]} || OutputProcess <- OutputProcess],
	loop(Id,ExoSelfProcess,CortexProcess,ActivationFunction,PF,AggrF,{MIProcessIDs,MIProcessIDs},[],{U_IWeightedInputProcessIDs,MWeightedInputProcess},OutputProcess,ROProcess).
...
perturb_IWeightedInputProcessIDs(Spread,MP,[{InputProcess,Weights}|WeightedInputProcess],Acc)->
	U_Weights = perturb_weights(Spread,MP,Weights,[]),
	perturb_IWeightedInputProcessIDs(Spread,MP,WeightedInputProcess,[{InputProcess,U_Weights}|Acc]);
