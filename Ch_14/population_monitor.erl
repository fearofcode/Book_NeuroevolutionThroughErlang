...
%% API
-export([init/2, start/0, start/1, start_link/0,
	 start_link/1, stop/0]).

%% gen_server callbacks
-export([code_change/3, continue/2, continue/3,
	 create_MutantAgentCopy/1, create_specie/3,
	 delete_population/1, extract_AgentIds/2, handle_call/3,
	 handle_cast/2, handle_info/2, init/1, init_population/2,
	 prep_PopState/2, terminate/2, test/0]).
...
-record(state,
	{op_mode = gt, population_id = test,
	 activeAgentWeightedInputs = [], agent_ids = [],
	 tot_agents, agents_left, op_tag, agent_summaries = [],
	 pop_gen = 0, eval_acc = 0, cycle_acc = 0, time_acc = 0,
	 tot_evaluations = 0, step_size, goal_status,
	 evolutionary_algorithm, fitness_postprocessor,
	 selection_algorithm, best_fitness,
	 survival_percentage = 5.0e-1, specie_size_limit = 10,
	 init_specie_size = 10, polis_id = mathema,
	 generation_limit = 100, evaluations_limit = 10000,
	 fitness_goal = inf, benchmarker_pid,
	 goal_reached = false}).
...
stop() -> gen_server:cast(monitor, {stop, normal}).
...
handle_call({stop, normal}, _From, S) ->
	...
	S#state.activeAgentWeightedInputs,
    [AgentProcess ! {self(), terminate}
     || {_DAgentID, AgentProcess}
	    <- ActiveAgentWeightedInputs],
    {stop, normal, S};
handle_call({stop, shutdown}, _From, State) ->
    {stop, shutdown,
     State}.%If the population_monitor process receives a {stop,normal} call, it checks if there are any still active agents. If there are any, it terminates them, and then itself terminates.
...
%%--------------------------------------------------------------------
handle_cast({AgentID, terminated, Fitness}, S)
    ...
    case AgentsLeft - 1 =< 0 of
      true ->
	  mutate_population(PopulationID,
	  ...
	  case OpTag of
	    continue ->
		SpecieIDs = (genotype:dirty_read({population,
						   PopulationID}))#population.specie_ids,
		SpecFitList = [(genotype:dirty_read({specie,
						     SpecieID}))#specie.fitness
			       || SpecieID <- SpecieIDs],
		BestFitness = lists:nth(1,
					lists:reverse(lists:sort([MaxFitness
								  || {_, _,
								      MaxFitness,
								      _}
									 <- SpecFitList]))),
		...
		case (U_PopGen >= Generation_Limit) or
		       (S#state.tot_evaluations >= Evaluation_Limit)
		       or (BestFitness >= Fitness_Goal)
		       or S#state.goal_reached
		    of
		  true ->%ENDING_CONDITION_REACHED
		      AgentIDs = extract_AgentIds(PopulationID, all),
		      TotalAgents = length(AgentIDs),
		      U_S = S#state{agent_ids = AgentIDs,
				    tot_agents = TotalAgents,
				    agents_left = TotalAgents,
				    pop_gen = U_PopGen},
		      {stop, normal, U_S};
		  false ->%IN_PROGRESS
		      AgentIDs = extract_AgentIds(PopulationID, all),
		      U_ActiveAgentWeightedInputs = summon_agents(OpMode,
								  AgentIDs),
		      TotalAgents = length(AgentIDs),
	  ...
	  ActiveAgentWeightedInputs =
	      S#state.activeAgentWeightedInputs,
	  U_ActiveAgentIDs = lists:keydelete(AgentID, 1,
					      ActiveAgentWeightedInputs),
	  U_S = S#state{activeAgentWeightedInputs =
			    U_ActiveAgentIDs,
...
handle_cast({AgentID, terminated, Fitness}, S)
    ...
    NewBestFitness = case Fitness >= S#state.best_fitness of
		       true -> Fitness;
		       false -> S#state.best_fitness
		     end,
    case (S#state.tot_evaluations >= Evaluation_Limit) or
	   (NewBestFitness > Fitness_Goal)
	   or S#state.goal_reached
	of
      true ->
	  case lists:keydelete(AgentID, 1,
			       S#state.activeAgentWeightedInputs)
	      of
	    [] ->
		U_S = S#state{activeAgentWeightedInputs = []},
	  ...
	  io:format("AgentID:~p of morphology:~p with fitness:~p "
		    "terminated.~n",
		    [AgentID, Morphology, Fitness]),
	  ...
	  Valid_AgentSummaries = case
				   length(ProperlySorted_AgentSummaries) >=
				     S#state.specie_size_limit
				     of
				   true ->
				       [{InvalidFitness, InvalidTotalN,
					 InvalidAgentID}
					| Remaining_AgentSummaries] =
				       ...
				       genotype:deleteAgent(InvalidAgentID,
							     safe),
				       lists:reverse(Remaining_AgentSummaries);
				   false -> ProperlySorted_AgentSummaries
				 end,
	  io:format("Valid_AgentSummaries:~p~n",
		    [Valid_AgentSummaries]),
	  {WinnerFitness, WinnerProfile, WinnerAgentID} =
	      selection_algorithm:SelectionAlgorithmName(Valid_AgentSummaries),
	  ActiveAgentIDP = case rand:uniform() < 1.0e-1 of
			      true ->
				  U_DeadPool_AgentSummaries =
				      lists:delete({WinnerFitness,
						    WinnerProfile,
						    WinnerAgentID},
						   Valid_AgentSummaries),
				  WinnerAgentProcess = AgentProcess =
							exoself:start(WinnerAgentID,
								      self()),
				  ...
				  U_DeadPool_AgentSummaries =
				      Valid_AgentSummaries,
				  CloneAgentID =
				      create_MutantAgentCopy(WinnerAgentID,
							     safe),
				  CloneAgentProcess = exoself:start(CloneAgentID,
								 self()),
				  {CloneAgentID, CloneAgentProcess}
			    end,
	  Top_AgentSummaries =
	      lists:sublist(U_DeadPool_AgentSummaries,
			    round(S#state.specie_size_limit *
				    S#state.survival_percentage)),
	  {_, _, TopAgentIDs} =
	      lists:unzip3(lists:sublist(Top_AgentSummaries, 3)),
	  %			io:format("TopAgentIDs:~p~n",[TopAgentIDs]),
	  USpecie = genotype:dirty_read({specie, SpecieID}),
	  genotype:write(USpecie#specie{dead_pool =
					    U_DeadPool_AgentSummaries,
					champion_ids = TopAgentIDs}),
	  ActiveAgentWeightedInputs =
	      S#state.activeAgentWeightedInputs,
	  U_ActiveAgentWeightedInputs = [ActiveAgentIDP
					 | lists:keydelete(AgentID, 1,
							   ActiveAgentWeightedInputs)],
	  U_S = S#state{activeAgentWeightedInputs =
			    U_ActiveAgentWeightedInputs,
...
handle_cast({op_tag, pause}, S)
    when S#state.op_tag == continue ->
    U_S = S#state{op_tag = pause}, {noreply, U_S};
...
handle_cast({op_tag, continue}, S)
    ...
    U_S = S#state{activeAgentWeightedInputs =
		      U_ActiveAgentWeightedInputs,
		  tot_agents = TotalAgents, agents_left = TotalAgents,
		  op_tag = continue},
    {noreply, U_S};
...
handle_cast({From, evaluations, SpecieID, AEA,
	     AgentCycleAcc, AgentTimeAcc},
	    S) ->
    ...
    U_S = case U_EvalAcc >= S#state.step_size of
	    ...
	    false ->
		S#state{eval_acc = U_EvalAcc, cycle_acc = U_CycleAcc,
			time_acc = U_TimeAcc,
			tot_evaluations = U_TotalEvaluations}
	  end,
    {noreply, U_S};
handle_cast({_From, goal_reached}, S) ->
    U_S = S#state{goal_reached = true}, {noreply, U_S};
handle_cast({_From, print_TRACE}, S) ->
    PopulationID = S#state.population_id,
    P = genotype:dirty_read({population, PopulationID}),
    io:format("******** TRACE ********:~n~p~n",
	      [P#population.trace]),
    {noreply, S};
...
terminate(Reason, S) ->
    case S of
      [] ->
	  io:format("******** Population_Monitor shut down "
		    "with Reason:~p, with State: []~n",
		    [Reason]);
      _ ->
	  OpMode = S#state.op_mode,
	  OpTag = S#state.op_tag,
	  ...
	  io:format("******** Population_Monitor:~p shut "
		    "down with Reason:~p OpTag:~p, while "
		    "in OpMode:~p~n",
		    [PopulationID, Reason, OpTag, OpMode]),
	  io:format("******** Total Agents:~p Population Generation:"
		    "~p Total_Evals:~p~n",
		    [S#state.tot_agents, S#state.pop_gen,
		     S#state.tot_evaluations]),
	  case S#state.benchmarker_pid of
	    undefined -> ok;
	    ProcessID -> ProcessID ! {S#state.population_id, completed, U_T}
	  end
    end.
...
extract_AgentIds(PopulationID, AgentType) ->
    ...
    case AgentType of
      champions -> extract_ChampionAgentIds(SpecieIDs, []);
      all -> extract_AllAgentIds(SpecieIDs, [])
    end.
...
extract_AllAgentIds([SpecieID | SpecieIDs], Acc) ->
    extract_AllAgentIds(SpecieIDs,
			extract_SpecieAgentIds(SpecieID) ++ Acc);
extract_AllAgentIds([], Acc) -> Acc.
...
summon_agents(OpMode, [AgentID | AgentIDs], Acc) ->
    ...
    summon_agents(OpMode, AgentIDs,
		  [{AgentID, AgentProcess} | Acc]);
summon_agents(_OpMode, [], Acc) ->
    Acc.%The summon_agents/2 and summon_agents/3 spawns all the agents in the Agent_ids list, and returns to the caller a list of tuples as follows: [{AgentID,AgentProcess}...].

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
test() -> init_population(#state{}, ?INIT_CONSTRAINTS).
...
prep_PopState(PMP, Specie_Constraints) ->
    S = #state{op_mode = PMP#pmp.op_mode,
...
init_population(Init_State, Specie_Constraints) ->
    ...
    F = fun () ->
		case genotype:read({population, PopulationID}) of
		  undefined ->
		      create_Population(PopulationID,
					Init_State#state.init_specie_size,
					Specie_Constraints);
		  _ ->
    ...
    case Result of
      {atomic, _} -> population_monitor:start(Init_State);
      Error ->
	  io:format("******** ERROR in PopulationMonitor:~p~n",
		    [Error])
    end.
...
create_Population(PopulationID, SpecieSize,
		  Specie_Constraints) ->
    SpecieIDs = [create_specie(PopulationID, SpecCon,
				origin, SpecieSize)
		  || SpecCon <- Specie_Constraints],
    ...
    Population = #population{id = PopulationID,
				 ...
				 C#constraint.population_fitness_postprocessor_f,
...
create_specie(PopulationID, SpeCon, Fingerprint) ->
    ...
    create_specie(PopulationID, SpecieID, 0, [], SpeCon,
		  Fingerprint).

create_specie(PopulationID, SpeCon, Fingerprint,
	      SpecieSize) ->
    SpecieID = genotype:generate_UniqueId(),
    create_specie(PopulationID, SpecieID, SpecieSize, [],
		  SpeCon, Fingerprint).

create_specie(PopulationID, SpecieID, 0, IdAcc,
	      SpeCon, Fingerprint) ->
    io:format("SpecieID:~p Morphology:~p~n",
	      [SpecieID, SpeCon#constraint.morphology]),
    Specie = #specie{id = SpecieID,
		     population_id = PopulationID,
		     fingerprint = Fingerprint, constraint = SpeCon,
		     agent_ids = IdAcc},
    genotype:write(Specie),
    SpecieID;
create_specie(PopulationID, SpecieID, Agent_Index,
	      IdAcc, SpeCon, Fingerprint) ->
    AgentID = {genotype:generate_UniqueId(), agent},
    genotype:constructAgent(SpecieID, AgentID, SpeCon),
    create_specie(PopulationID, SpecieID, Agent_Index - 1,
		  [AgentID | IdAcc], SpeCon, Fingerprint).
...
mutate_population(PopulationID, KeepTotal,
		  Fitness_Postprocessor, Selection_Algorithm) ->
    NeuralEnergyCost = calculate_EnergyCost(PopulationID),
    F = fun () ->
		P = genotype:read({population, PopulationID}),
		SpecieIDs = P#population.specie_ids,
		[mutate_Specie(SpecieID, KeepTotal, NeuralEnergyCost,
			       Fitness_Postprocessor, Selection_Algorithm)
		 || SpecieID <- SpecieIDs]
	end,
...
mutate_Specie(SpecieID, PopulationLimit,
	      NeuralEnergyCost, Fitness_Postprocessor_Name,
	...
	lists:reverse(lists:sort(constructAgentSummaries(AgentIDs,
							  []))),
    io:format("Using: Fitness Postprocessor:~p Selection "
	      "Algorirthm:~p~n",
	...
	selection_algorithm:Selection_Algorithm_Name(ProperlySorted_AgentSummaries,
						     NeuralEnergyCost,
						     PopulationLimit),
    {FList, _TNList, _AgentIds} =
	lists:unzip3(Sorted_AgentSummaries),
    [TopFitness | _] = FList,
    {Factor, Fitness} = S#specie.innovation_factor,
    U_InnovationFactor = case TopFitness > Fitness of
			   true -> {0, TopFitness};
			   false -> {Factor - 1, Fitness}
			 end,
    genotype:write(S#specie{agent_ids = NewGenAgentIDs,
			    champion_ids = TopAgentIDs,
...
constructAgentSummaries([AgentID | AgentIDs], Acc) ->
    A = genotype:dirty_read({agent, AgentID}),
    constructAgentSummaries(AgentIDs,
			     [{A#agent.fitness,
			       length((genotype:dirty_read({cortex,
							    A#agent.cortex_id}))#cortex.neuron_ids),
			       AgentID}
			      | Acc]);
constructAgentSummaries([], Acc) -> Acc.
...
calculate_EnergyCost(PopulationID) ->
    AgentIDs = extract_AgentIds(PopulationID, all),
    TotalEnergy = lists:sum([extract_AgentFitness(AgentID)
			   || AgentID <- AgentIDs]),
    TotalNeurons =
	lists:sum([extract_AgentTotalNeurons(AgentID)
		   || AgentID <- AgentIDs]),
...
calculate_fitness([AgentID | AgentIDs], FitnessAcc) ->
    ...
    case A#agent.fitness of
      undefined -> calculate_fitness(AgentIDs, FitnessAcc);
      Fitness ->
	  calculate_fitness(AgentIDs, [Fitness | FitnessAcc])
    end;
calculate_fitness([], FitnessAcc) -> FitnessAcc.
...
gather_STATS(PopulationID, EvaluationsAcc) ->
    io:format("Gathering Species STATS in progress~n"),
    TimeStamp = now(),
    F = fun () ->
		P = genotype:read({population, PopulationID}),
		T = P#population.trace,
		SpecieSTATS = [update_SpecieSTAT(SpecieID, TimeStamp)
			       || SpecieID <- P#population.specie_ids],
		PopulationSTATS = T#trace.stats,
		U_PopulationSTATS = [SpecieSTATS | PopulationSTATS],
		U_TotalEvaluations = T#trace.tot_evaluations +
				     EvaluationsAcc,
		U_Trace = T#trace{stats = U_PopulationSTATS,
				  tot_evaluations = U_TotalEvaluations},
...
update_SpecieSTAT(SpecieID, TimeStamp) ->
    Specie_Evaluations = get({evaluations, SpecieID}),
    put({evaluations, SpecieID}, 0),
    S = genotype:read({specie, SpecieID}),
    {Avg_Neurons, Neurons_Std} =
	calculate_SpecieAvgNodes({specie, S}),
    {AvgFitness, Fitness_Std, MaxFitness, MinFitness} =
	calculate_SpecieFitness({specie, S}),
    SpecieDiversity = calculate_SpecieDiversity({specie,
						 S}),
    STAT = #stat{morphology =
		     (S#specie.constraint)#constraint.morphology,
		 specie_id = SpecieID, avg_neurons = Avg_Neurons,
		 std_neurons = Neurons_Std, avg_fitness = AvgFitness,
		 std_fitness = Fitness_Std, max_fitness = MaxFitness,
...
calculate_SpecieAvgNodes(SpecieID) ->
    io:format("calculate_SpecieAvgNodes(SpecieID):~p~n",
	      [SpecieID]),
    S = genotype:read({specie, SpecieID}),
    calculate_SpecieAvgNodes({specie, S}).

calculate_AvgNodes([AgentID | AgentIDs], NAcc) ->
    io:format("calculate_AvgNodes/2 AgentID:~p~n",
	      [AgentID]),
...
calculate_diversity([AgentID | AgentIDs],
		    DiversityAcc) ->
    A = genotype:read({agent, AgentID}),
    Fingerprint = A#agent.fingerprint,
    U_DiversityAcc = (DiversityAcc -- [Fingerprint]) ++
		       [Fingerprint],
