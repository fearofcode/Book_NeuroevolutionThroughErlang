...
const(Parameter, _NeuronIDs, _Generation) ->
    ConstMaxAttempts = Parameter, ConstMaxAttempts.

%const/3 returns the preset const max_attempts value.

wsize_proportional(Parameter, NeuronIDs, Generation) ->
    Power = Parameter,
    ActiveNeuronIDs = extract_RecGenNeuronIDs(NeuronIDs,
					      Generation, 3, []),
    Total_ActiveNeuron_Weights =
	extract_NWeightCount(ActiveNeuronIDs, 0),
    20 +
      functions:sat(round(math:pow(Total_ActiveNeuron_Weights,
				   Power)),
		    100, 0).
...
extract_RecGenNeuronIDs([NeuronID | NeuronIDs], Generation,
			AgeLimit, Acc) ->
    Neuron = genotype:dirty_read({neuron, NeuronID}),
    NeuronGen = N#neuron.generation,
    case NeuronGen >= Generation - AgeLimit of
      true ->
	  extract_RecGenNeuronIDs(NeuronIDs, Generation, AgeLimit,
				  [NeuronID | Acc]);
      false ->
	  extract_RecGenNeuronIDs(NeuronIDs, Generation, AgeLimit,
				  Acc)
    end;
extract_RecGenNeuronIDs([], _Generation, _AgeLimit,
			Acc) ->
    Acc.
...
extract_NWeightCount([NeuronID | RecGenNeuronIDs], Acc) ->
    ...
    TotalWeights = lists:sum([length(Weights)
			    || {_IId, Weights} <- WeightedInputs]),
    extract_NWeightCount(RecGenNeuronIDs, TotalWeights + Acc);
extract_NWeightCount([], Acc) -> Acc.
...
nsize_proportional(Parameter, NeuronIDs, Generation) ->
    Power = Parameter,
    Total_Neurons = length(extract_RecGenNeuronIDs(NeuronIDs,
						 Generation, 3, [])),
    20 +
      functions:sat(round(math:pow(Total_Neurons, Power)), 100,
		    0).%nsize_proportional/3 calculates the max_attempts to be proportional to the number of neurons which were within the last 3 generations mutated or added to the NN.
