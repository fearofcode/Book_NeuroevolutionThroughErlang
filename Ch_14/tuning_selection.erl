...
dynamic(NeuronIDs, AgentGeneration, PerturbationRange,
	AnnealingParameter) ->
    AgeLimit = math:sqrt(1 / rand:uniform()),
    ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(NeuronIDs,
						     AgentGeneration, AgeLimit,
						     PerturbationRange,
						     AnnealingParameter, [])
				of
			      [] ->
...
extract_CurGenNeuronIDPs([NeuronID | NeuronIDs], Generation,
		    AgeLimit, PR, AP, Acc) ->
    ...
    case NeuronGen >= Generation - AgeLimit of
	  ...
	  Spread = PR * math:pi() *
		     math:pow(AP, Age),%math:pi()*math:pow(0.5,Age),
	  extract_CurGenNeuronIDPs(NeuronIDs, Generation, AgeLimit, PR, AP,
			      [{NeuronID, Spread} | Acc]);
      false ->
	  extract_CurGenNeuronIDPs(NeuronIDs, Generation, AgeLimit, PR, AP,
			      Acc)
    end;
extract_CurGenNeuronIDPs([], _Generation, _AgeLimit, _PR,
		    _AP, Acc) ->
    Acc.
...
dynamic_random(NeuronIDs, AgentGeneration,
...
choose_randomNeuronIDPs(MutationP, NWeightedInputs) ->
    case choose_randomNeuronIDPs(NWeightedInputs, MutationP, [])
	of
      [] ->
	  {NeuronID, Spread} =
	      lists:nth(rand:uniform(length(NWeightedInputs)),
			NWeightedInputs),
	  [{NeuronID, Spread}];
      Acc -> Acc
    end.

choose_randomNeuronIDPs([{NeuronID, Spread} | NWeightedInputs],
		   MutationP, Acc) ->
    U_Acc = case rand:uniform() < MutationP of
	      true -> [{NeuronID, Spread} | Acc];
	      false -> Acc
	    end,
    choose_randomNeuronIDPs(NWeightedInputs, MutationP, U_Acc);
choose_randomNeuronIDPs([], _MutationP, Acc) -> Acc.
...
active(NeuronIDs, AgentGeneration, PerturbationRange,
       AnnealingParameter) ->
    extract_CurGenNeuronIDPs(NeuronIDs, AgentGeneration, 3,
			PerturbationRange, AnnealingParameter, []).

%active/4 selection algorithm composes a neuron id pool from all neurons who are younger than 3 generations.

active_random(NeuronIDs, AgentGeneration, PerturbationRange,
	      AnnealingParameter) ->
    ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(NeuronIDs,
						     AgentGeneration, 3,
						     PerturbationRange,
...
current(NeuronIDs, AgentGeneration, PerturbationRange,
	AnnealingParameter) ->
    case extract_CurGenNeuronIDPs(NeuronIDs, AgentGeneration, 0,
			     PerturbationRange, AnnealingParameter, [])
	of
      [] ->
	  [NeuronID | _] = NeuronIDs,
...
current_random(NeuronIDs, AgentGeneration,
	       PerturbationRange, AnnealingParameter) ->
    ChosenNWeightedInputs = case extract_CurGenNeuronIDPs(NeuronIDs,
						     AgentGeneration, 0,
						     PerturbationRange,
...
all(NeuronIDs, AgentGeneration, PerturbationRange,
    AnnealingParameter) ->
    extract_CurGenNeuronIDPs(NeuronIDs, AgentGeneration,
			AgentGeneration, PerturbationRange, AnnealingParameter,
			[]).

%all/4 returns a list of tuples composed of all ids (and their spread values) belonging to the NN, to the caller.

all_random(NeuronIDs, AgentGeneration, PerturbationRange,
	   AnnealingParameter) ->
    ChosenNWeightedInputs = extract_CurGenNeuronIDPs(NeuronIDs,
						AgentGeneration,
						AgentGeneration,
						PerturbationRange,
						AnnealingParameter, []),
    Total_Neurons = length(ChosenNWeightedInputs),
    MutationP = 1 / math:sqrt(Total_Neurons),
    choose_randomNeuronIDPs(MutationP,
		       ChosenNWeightedInputs).%all_random/4 first composes a list of tuples from neuron_ids and their spreads, and then creates a sublist by choosing each element with a probability of 1/math:sqrt(Total_neurons).
