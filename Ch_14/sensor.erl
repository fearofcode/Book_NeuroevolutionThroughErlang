...
prep(ExoSelfProcess) ->
    receive
      {ExoSelfProcess,
       {Id, CortexProcess, Scape, SensorName, VL, Parameters,
	FanoutProcess}} ->
	  loop(Id, ExoSelfProcess, CortexProcess, Scape, SensorName, VL,
	       Parameters, FanoutProcess)
    end.
...
loop(Id, ExoSelfProcess, CortexProcess, Scape, SensorName, VL,
    ...
    receive
      {CortexProcess, sync} ->
	  SensoryVector = sensor:SensorName(VL, Parameters,
					    Scape),
	  [Pid ! {self(), forward, SensoryVector}
	   || Pid <- FanoutProcess],
	  loop(Id, ExoSelfProcess, CortexProcess, Scape, SensorName, VL,
	       Parameters, FanoutProcess);
...
rng(VL, _Scape) -> rng1(VL, []).

rng1(0, Acc) -> Acc;
rng1(VL, Acc) -> rng1(VL - 1, [rand:uniform() | Acc]).
...
xor_GetInput(VL, _Parameters, Scape) ->
    ...
    receive
	  ...
	  case length(SensoryVector) == VL of
	    true -> SensoryVector;
	    false ->
		io:format("Error in sensor:xor_sim/3, VL:~p SensoryVecto"
			  "r:~p~n",
			  [VL, SensoryVector]),
...
pb_GetInput(VL, Parameters, Scape) ->
    ...
    receive
	  ...
	  case length(SensoryVector) == VL of
	    true -> SensoryVector;
	    false ->
		io:format("Error in sensor:pb_GetInput/3, VL:~p "
			  "SensoryVector:~p~n",
			  [VL, SensoryVector]),
		lists:duplicate(VL, 0)
	  end
