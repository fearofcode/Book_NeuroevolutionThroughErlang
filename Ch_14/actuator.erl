...
prep(ExoSelfProcess) ->
    receive
      {ExoSelfProcess,
       {Id, CortexProcess, Scape, ActuatorName, Parameters,
	FaninProcess}} ->
	  loop(Id, ExoSelfProcess, CortexProcess, Scape, ActuatorName,
	       Parameters, {FaninProcess, FaninProcess}, [])
    end.

%When gen/2 is executed it spawns the actuator element and immediately begins to wait for its initial state message.

loop(Id, ExoSelfProcess, CortexProcess, Scape, ActuatorName, Parameters,
     {[FromProcess | FaninProcess], MFaninProcess}, Acc) ->
    receive
      {FromProcess, forward, Input} ->
	  loop(Id, ExoSelfProcess, CortexProcess, Scape, ActuatorName, Parameters,
	       {FaninProcess, MFaninProcess}, Input ++ Acc);
...
loop(Id, ExoSelfProcess, CortexProcess, Scape, ActuatorName, Parameters,
     {[], MFaninProcess}, Acc) ->
    {Fitness, EndFlag} = actuator:ActuatorName(lists:reverse(Acc),
					Parameters, Scape),
    CortexProcess ! {self(), sync, Fitness, EndFlag},
    loop(Id, ExoSelfProcess, CortexProcess, Scape, ActuatorName, Parameters,
	 {MFaninProcess, MFaninProcess},
