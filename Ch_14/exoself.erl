...
-record(state,
	{agent_id, generation, pm_pid, neuron_process_ids, cx_pid,
	 specie_id, spids = [], npids = [], neuron_ids = [],
	 apids = [], scape_pids = [], highest_fitness = -1,
	 eval_acc = 0, cycle_acc = 0, time_acc = 0,
	 max_attempts = 10, attempt = 1, tuning_duration_f,
	 tuning_selection_f, annealing_parameter,
	 perturbation_range}).
...
start(AgentID) ->
    case whereis(monitor) of
      ...
      ProcessID -> start(AgentID, ProcessID)
    end.
...
prep(AgentID, PMProcess) ->
    ...
    {TuningDurationFunction, Parameter} =
	A#agent.tuning_duration_f,
    S = #state{agent_id = AgentID,
	       generation = A#agent.generation, pm_pid = PMProcess,
	       neuron_process_ids = NeuronPIDs, cx_pid = CortexProcess,
	       specie_id = A#agent.specie_id, spids = SensorProcess,
	       npids = NeuronProcess, neuron_ids = NeuronIDs, apids = ActuatorProcess,
	       scape_pids = ScapeProcessIDs,
	       max_attempts =
		   tuning_duration:TuningDurationFunction(Parameter,
							  NeuronIDs,
							  A#agent.generation),
...
loop(S) ->
    receive
	  ...
	  {U_HighestFitness, U_Attempt} = case Fitness >
						 S#state.highest_fitness
					      of
					    true ->
						[NeuronProcess ! {self(), weight_backup}
						 || NeuronProcess <- S#state.npids],
						{Fitness, 0};
					    false ->
						Perturbed_NeuronIDPs =
						    get(perturbed),
						[ets:lookup_element(NeuronPIDs,
								    NeuronID, 2)
						   ! {self(), weight_restore}
						 || {NeuronID, _Spread}
							<- Perturbed_NeuronIDPs],
	  ...
	  gen_server:cast(S#state.pm_pid,
			  {self(), evaluations, S#state.specie_id, 1, Cycles,
			   Time}),
	  case (U_Attempt >= S#state.max_attempts) or
		...
		terminate_phenotype(S#state.cx_pid, S#state.spids,
				    S#state.npids, S#state.apids,
				    S#state.scape_pids),
		io:format("Agent:~p terminating. Genotype has been "
			  "backed up.~n Fitness:~p~n TotalEvaluations:~p~n "
			  "TotalCycles:~p~n TimeAcc:~p~n",
			  [self(), U_HighestFitness, U_EvalAcc, U_CycleAcc,
			   U_TimeAcc]),
		...
		ChosenNeuronIDPs =
		    tuning_selection:TuningSelectionFunction(S#state.neuron_ids,
							     S#state.generation,
							     PerturbationRange,
							     AnnealingParameter),
		[ets:lookup_element(NeuronPIDs, NeuronID, 2) !
		   {self(), weight_perturb, Spread}
		 || {NeuronID, Spread} <- ChosenNeuronIDPs],
		%io:format("ChosenNeuronProcess:~p~n",[ChosenNeuronIDPs]),
		put(perturbed, ChosenNeuronIDPs),
		CortexProcess ! {self(), reactivate},
		U_S = S#state{cycle_acc = U_CycleAcc,
			      time_acc = U_TimeAcc, eval_acc = U_EvalAcc,
			      attempt = U_Attempt,
...
spawnCerebralUnits(NeuronPIDs, CerebralUnitType,
		    [Id | Ids]) ->
...
spawn_Scapes(NeuronPIDs, SensorIDs, ActuatorIDs) ->
    Sensor_Scapes = [(genotype:dirty_read({sensor,
					   Id}))#sensor.scape
		     || Id <- SensorIDs],
    Actuator_Scapes = [(genotype:dirty_read({actuator,
					     Id}))#actuator.scape
		       || Id <- ActuatorIDs],
    %io:format("Sensor_Scapes:~p~n Actuator_Scapes:~p~n",[Sensor_Scapes,Actuator_Scapes]),
    Unique_Scapes = Sensor_Scapes ++
		      Actuator_Scapes -- Sensor_Scapes,
    %io:format("Unique_Scapes:~p~n",[Unique_Scapes]),
    SN_Tuples = [{scape:gen(self(), node()), ScapeName}
		 || {private, ScapeName} <- Unique_Scapes],
    [ets:insert(NeuronPIDs, {ScapeName, ProcessID})
     || {ProcessID, ScapeName} <- SN_Tuples],
    [ets:insert(NeuronPIDs, {ProcessID, ScapeName})
     || {ProcessID, ScapeName} <- SN_Tuples],
    [ProcessID ! {self(), ScapeName}
     || {ProcessID, ScapeName} <- SN_Tuples],
...
link_Sensors([SensorID | SensorIDs], NeuronPIDs) ->
    ...
    CortexProcess = ets:lookup_element(NeuronPIDs, S#sensor.cortex_id,
				2),
    SensorName = S#sensor.name,
    FanoutIDs = S#sensor.fanout_ids,
    FanoutProcess = [ets:lookup_element(NeuronPIDs, Id, 2)
		   || Id <- FanoutIDs],
    ...
    SensorProcess !
      {self(),
       {SensorID, CortexProcess, Scape, SensorName, S#sensor.vector_length,
	S#sensor.parameters, FanoutProcess}},
    link_Sensors(SensorIDs, NeuronPIDs);
link_Sensors([], NeuronPIDs) -> ok.
...
link_Actuators([ActuatorID | ActuatorIDs], NeuronPIDs) ->
    ...
    CortexProcess = ets:lookup_element(NeuronPIDs, A#actuator.cortex_id,
				2),
    ActuatorName = A#actuator.name,
    FaninIDs = A#actuator.fanin_ids,
    FaninProcess = [ets:lookup_element(NeuronPIDs, Id, 2)
		  || Id <- FaninIDs],
    ...
    ActuatorProcess !
      {self(),
       {ActuatorID, CortexProcess, Scape, ActuatorName, A#actuator.parameters,
	FaninProcess}},
    link_Actuators(ActuatorIDs, NeuronPIDs);
link_Actuators([], NeuronPIDs) -> ok.
...
link_Neurons([NeuronID | NeuronIDs], NeuronPIDs) ->
    ...
    CortexProcess = ets:lookup_element(NeuronPIDs, N#neuron.cortex_id,
				2),
    ...
    WeightedInputProcess = convertWeightedInputsToWeightedInputProcessIDs(NeuronPIDs,
					      WeightedInputs, []),
    OutputProcess = [ets:lookup_element(NeuronPIDs, Id, 2)
		   || Id <- OutputIDs],
    ROProcess = [ets:lookup_element(NeuronPIDs, Id, 2)
	       || Id <- ROIDs],
    NeuronProcess !
      {self(),
       {NeuronID, CortexProcess, ActivationFunctionName, PFName, AggrFName, WeightedInputProcess,
	OutputProcess, ROProcess}},
    link_Neurons(NeuronIDs, NeuronPIDs);
link_Neurons([], NeuronPIDs) -> ok.
...
convertWeightedInputsToWeightedInputProcessIDs(NeuronPIDs,
			    [{Id, Weights} | FaninWeightedInputs], Acc) ->
    convertWeightedInputsToWeightedInputProcessIDs(NeuronPIDs,
				FaninWeightedInputs,
				[{ets:lookup_element(NeuronPIDs, Id, 2), Weights}
				 | Acc]);
...
updateGenotype(NeuronPIDs, [{NeuronID, WeightedInputProcessIDs} | WeightPs]) ->
    Neuron = genotype:dirty_read({neuron, NeuronID}),
    UpdatedWeightedInput = convertWeightedInputProcessesToWeightedInputs(NeuronPIDs,
					      WeightedInputProcessIDs, []),
...
convertWeightedInputProcessesToWeightedInputs(NeuronPIDs,
		   [{ProcessID, Weights} | WeightedInputProcess], Acc) ->
    convertWeightedInputProcessesToWeightedInputs(NeuronPIDs, WeightedInputProcess,
		       [{ets:lookup_element(NeuronPIDs, ProcessID, 2), Weights}
			| Acc]);
...
terminate_phenotype(CortexProcess, SensorProcess, NeuronProcess, ActuatorProcess,
		    ScapeProcessIDs) ->
...
gather_acks(ProcessIDIndex) ->
    receive
      {_From, ready} -> gather_acks(ProcessIDIndex - 1)
      after 100000 ->
		io:format("******** Not all acks received:~p~n",
			  [ProcessIDIndex])
    end.
