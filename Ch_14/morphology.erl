...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% MORPHOLOGIES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
xor_mimic(sensors) ->
    [#sensor{name = xor_GetInput,
	     scape = {private, xor_sim}, vector_length = 2}];
xor_mimic(actuators) ->
    [#actuator{name = xor_SendOutput,
	       scape = {private, xor_sim}, vector_length = 1}].
...
pole_balancing(sensors) ->
    [#sensor{name = pb_GetInput, scape = {private, pb_sim},
	     vector_length = 3, parameters = [3]}];
pole_balancing(actuators) ->
    [#actuator{name = pb_SendOutput,
	       scape = {private, pb_sim}, vector_length = 1,
...
discrete_tmaze(sensors) ->
    [#sensor{name = dtm_GetInput,
	     scape = {private, dtm_sim}, vector_length = 4,
	     parameters = [all]}];
discrete_tmaze(actuators) ->
    [#actuator{name = dtm_SendOutput,
	       scape = {private, dtm_sim}, vector_length = 1, parameters = []}].
