...
-define(SEARCH_PARAMTERS_MUTATION_PROBABILITY, 1.0e-1).

-define(MUTATORS,
	[mutate_weights, add_bias, remove_bias, mutate_af,
	 add_outlink,
	 %	remove_outlink,
	 add_inlink,
	 %	remove_inlink,
...
-define(ES_MUTATORS,
	[mutate_tuning_selection, mutate_tuning_duration,
	 mutate_tuning_annealing,
	 mutate_tot_topological_mutations]).

-define(ACTUATORS,
	morphology:get_Actuators(A#agent.morphology)).

-define(SENSORS,
	morphology:get_Sensors(A#agent.morphology)).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
long_test(TotalMutations) when TotalMutations > 0 ->
    genotype:create_test(), short_test(TotalMutations).

short_test(0) -> exoself:start(test, void);
short_test(Index) -> test(), short_test(Index - 1).
...
test(AgentID, Mutator) ->
    F = fun () -> genome_mutator:Mutator(AgentID) end,
...
mutate(AgentID) ->
    ...
    F = fun () ->
		...
		TotalMutations =
		    tot_topological_mutations:TTM_Name(Parameter, AgentID),
...
mutate_SearchParameters(AgentID) ->
    case rand:uniform() <
	   (?SEARCH_PARAMTERS_MUTATION_PROBABILITY)
	of
      true ->
...
apply_ESMutators(_AgentID, 0) -> done;
apply_ESMutators(AgentID, MutationIndex) ->
    ES_Mutators = (?ES_MUTATORS),
    ES_Mutator =
	lists:nth(rand:uniform(length(ES_Mutators)),
		  ES_Mutators),
    io:format("Evolutionary Strategy Mutation Operator:~p~n",
	      [ES_Mutator]),
    ...
    case Result of
      ...
      Error ->
	  io:format("******** Error:~p~nRetrying with new "
		    "Mutation...~n",
		    [Error]),
	  apply_ESMutators(AgentID, MutationIndex - 1)
    end.
...
apply_Mutators(_AgentID, 0) -> done;
...
mutate_tuning_selection(AgentID) ->
    A = genotype:read({agent, AgentID}),
    case (A#agent.constraint)#constraint.tuning_selection_fs
	   -- [A#agent.tuning_selection_f]
	of
      [] ->
	  exit("********ERROR:mutate_tuning_selection/1:: "
	       "Nothing to mutate, only a single function "
	       "available.");
      Tuning_Selection_Functions ->
	  New_TSF =
	      lists:nth(rand:uniform(length(Tuning_Selection_Functions)),
			Tuning_Selection_Functions),
...
mutate_tuning_annealing(AgentID) ->
    A = genotype:read({agent, AgentID}),
    case
      (A#agent.constraint)#constraint.annealing_parameters --
	[A#agent.annealing_parameter]
	of
      [] ->
	  exit("********ERROR:mutate_tuning_annealing/1:: "
	       "Nothing to mutate, only a single function "
	       "available.");
      Tuning_Annealing_Parameters ->
	  New_TAP =
	      lists:nth(rand:uniform(length(Tuning_Annealing_Parameters)),
			Tuning_Annealing_Parameters),
...
perturbWeightedInputs(WeightedInputs) ->
    Total_Weights = lists:sum([length(Weights)
			     || {_InputID, Weights} <- WeightedInputs]),
    MP = 1 / math:sqrt(Total_Weights),
    perturbWeightedInputs(MP, WeightedInputs, []).

perturbWeightedInputs(MP,
		      [{InputID, Weights} | WeightedInputs], Acc) ->
    U_Weights = perturb_weights(MP, Weights, []),
    perturbWeightedInputs(MP, WeightedInputs,
			  [{InputID, U_Weights} | Acc]);
...
perturb_weights(MP, [W | Weights], Acc) ->
    U_W = case rand:uniform() < MP of
	    true ->
		sat((rand:uniform() - 5.0e-1) * (?DELTA_MULTIPLIER) + W,
		    -(?SAT_LIMIT), ?SAT_LIMIT);
	    false -> W
	  end,
    perturb_weights(MP, Weights, [U_W | Acc]);
perturb_weights(_MP, [], Acc) -> lists:reverse(Acc).
...
add_bias(AgentID) ->
    ...
    Neuron = genotype:read({neuron, NeuronID}),
    WeightedInputs = N#neuron.weighted_inputs,
    case lists:keymember(bias, 1, WeightedInputs) of
      true ->
	  exit("********ERROR:add_bias:: This Neuron "
	       "already has a bias part.");
      false ->
	  U_WeightedInputs = WeightedInputs ++
			       [{bias, [rand:uniform() - 5.0e-1]}],
	  UpdatedNeuron = Neuron#neuron{weighted_inputs = U_WeightedInputs,
			 generation = Generation},
	  EvoHist = A#agent.evo_hist,
	  U_EvoHist = [{add_bias, NeuronID} | EvoHist],
	  UpdatedActuator = A#agent{evo_hist = U_EvoHist},
	  genotype:write(UpdatedNeuron),
	  genotype:write(UpdatedActuator)
    end.

%The add_bias/1 function is called with the AgentID parameter. The function first extracts the neuron_ids list from the cortex element and chooses a random neuron from the id list. The neuron is the read from the database and its weighted_inputs list checked for bias element. If the neuron's weighted_inputs list already has a bias tuple, then the function is exited. If the weighted_inputs list does not have the bias tuple, then the bias is added, the agent's evolutionary history list is updated with the tuple {add_bias,NeuronID}, and the updated neuron and agent records are written to the database.
...
remove_bias(AgentID) ->
    ...
    case lists:keymember(bias, 1, WeightedInputs) of
      false ->
	  exit("********ERROR:remove_bias:: This Neuron "
	       "does not have a bias part.");
      true ->
	  U_WeightedInputs = lists:keydelete(bias, 1,
					     WeightedInputs),
	  UpdatedNeuron = Neuron#neuron{weighted_inputs = U_WeightedInputs,
			 generation = Generation},
	  EvoHist = A#agent.evo_hist,
	  U_EvoHist = [{remove_bias, NeuronID} | EvoHist],
	  UpdatedActuator = A#agent{evo_hist = U_EvoHist},
	  genotype:write(UpdatedNeuron),
	  genotype:write(UpdatedActuator)
    end.

%The remove_bias/1 function is called with the AgentID parameter. The function first extracts the neuron_ids list from the cortex element and chooses a random neuron from the id list. The neuron is the read from the database and its weighted_inputs list checked for bias element. If the neuron's weighted_inputs list has a bias tuple, it is removed ,the agent's evolutionary history list is updated with the tuple {add_bias,NeuronID}, and the updated neuron and agent records are written to the database. If the weighted_inputs list does not have the bias tuple, the function exits with an error stating so.
...
mutate_af(AgentID) ->
    ...
    case (A#agent.constraint)#constraint.neural_afs -- [ActivationFunction]
	of
      [] ->
	  exit("********ERROR:mutate_af:: There are "
	       "no other activation functions to use.");
      Activation_Functions ->
	  NewActivationFunction =
	      lists:nth(rand:uniform(length(Activation_Functions)),
			Activation_Functions),
...
mutate_pf(AgentID) ->
    ...
    Neuron = genotype:read({neuron, NeuronID}),
    PF = N#neuron.pf,
    case (A#agent.constraint)#constraint.neural_pfs -- [PF]
	of
      [] ->
	  exit("********ERROR:mutate_pf:: There are "
	       "no other plasticity functions to use.");
      Plasticity_Functions ->
	  NewPF =
	      lists:nth(rand:uniform(length(Plasticity_Functions)),
			Plasticity_Functions),
	  UpdatedNeuron = Neuron#neuron{pf = NewPF, generation = Generation},
	  EvoHist = A#agent.evo_hist,
...
mutate_aggrf(AgentID) ->
    ...
    case (A#agent.constraint)#constraint.neural_aggr_fs --
	   [AggrF]
	of
      [] ->
	  exit("********ERROR:mutate_aggrf:: There are "
	       "no other aggregation functions to use.");
      Aggregation_Functions ->
	  NewAggrF =
	      lists:nth(rand:uniform(length(Aggregation_Functions)),
			Aggregation_Functions),
...
link_FromElementToElement(AgentID, From_ElementId,
			  To_ElementId) ->
    case {From_ElementId, To_ElementId} of
      {{_FromSensorID, neuron}, {_ToSensorID, neuron}} ->
	  link_FromNeuronToNeuron(AgentID, From_ElementId,
				  To_ElementId);
      {{_FromSensorID, sensor}, {_ToSensorID, neuron}} ->
	  link_FromSensorToNeuron(AgentID, From_ElementId,
				  To_ElementId);
      {{_FromNeuronID, neuron}, {_ToActuatorID, actuator}} ->
	  link_FromNeuronToActuator(AgentID, From_ElementId,
				    To_ElementId)
    end.
...
link_FromNeuronToNeuron(AgentID, From_NeuronId,
			To_NeuronId) ->
    A = genotype:read({agent, AgentID}),
    Generation = A#agent.generation,
    %From Partf
    FromN = genotype:read({neuron, From_NeuronId}),
    U_FromN = link_FromNeuron(FromN, To_NeuronId,
			      Generation),
    genotype:write(U_FromN),
    %To Part
    ToN = genotype:read({neuron,
			 To_NeuronId}),%We read it afterwards, in the case that it's the same Element. Thus we do not overwrite the earlier changes.
    FromOVL = 1,
    U_ToN = link_ToNeuron(From_NeuronId, FromOVL, ToN,
			  Generation),
...
link_FromNeuron(FromN, ToId, Generation) ->
    ...
    case lists:member(ToId, FromOutputIDs) of
      true ->
	  exit("******** ERROR:add_NeuronO[can not add "
	       "OID to Neuron]: ~p already a member "
	       "of ~p~n",
	  ...
	  FromN#neuron{output_ids = U_FromOutputIDs,
		       ro_ids = U_FromROIDs, generation = Generation}
    end.
...
link_ToNeuron(FromId, FromOVL, ToN, Generation) ->
    ToWeightedInputs = ToN#neuron.weighted_inputs,
    case lists:keymember(FromId, 1, ToWeightedInputs) of
      true ->
	  exit("ERROR:add_NeuronI::[can not add IID]: "
	       "~p already a member of ~p~n",
	       [FromId, ToN#neuron.id]);
      false ->
	  U_ToWeightedInputs = [{FromId,
				 genotype:createNeuralWeights(FromOVL, [])}
				| ToWeightedInputs],
	  ToN#neuron{weighted_inputs = U_ToWeightedInputs,
		     generation = Generation}
    end.

%link_ToNeuron/4 updates the record of ToN, so that its updated to receive a connection from the element FromId. The link eminates from element with the id FromId, whose output vector length is FromOVL, and the connection is made to the neuron ToN, the record whose is updated in this function. The ToN's weighted_inputs is updated with the tuple {FromId,[W_1...W_FromOVL]}, then the neuron's generation is updated to Generation (the current, most recent generation), and the updated ToN's record is returned to the caller. On the other hand, if the FromId is already part of the ToN's weighted_inputs list, which means that the link already exists between the neuron ToN, and element FromId, then the function exits with an error.

link_FromSensorToNeuron(AgentID, From_SensorId,
			To_NeuronId) ->
    A = genotype:read({agent, AgentID}),
    Generation = A#agent.generation,
    %From Part
    ...
    U_ToN = link_ToNeuron(From_SensorId, FromOVL, ToN,
			  Generation),
    genotype:write(U_ToN).
...
link_FromSensor(FromS, ToId, Generation) ->
    ...
    case lists:member(ToId, FromFanoutIDs) of
      true ->
	  exit("******** ERROR:link_FromSensor[can not "
	       "add ToId to Sensor]: ~p already a member "
	       "of ~p~n",
	       [ToId, FromS#sensor.id]);
      false ->
	  FromS#sensor{fanout_ids = [ToId | FromFanoutIDs],
		       generation = Generation}
    end.
...
link_FromNeuronToActuator(AgentID, From_NeuronId,
			  To_ActuatorId) ->
    A = genotype:read({agent, AgentID}),
    Generation = A#agent.generation,
    %From Part
    FromN = genotype:read({neuron, From_NeuronId}),
    U_FromN = link_FromNeuron(FromN, To_ActuatorId,
			      Generation),
    genotype:write(U_FromN),
    %To Part
    ...
    case length(FaninIDs) >= ToA#actuator.vector_length of
      true ->
	  exit("******** ERROR:link_FromNeuronToActuator:: "
	       "Actuator already fully connected");
      false ->
	  U_FaninIDs = [From_NeuronId | FaninIDs],
	  genotype:write(ToA#actuator{fanin_ids = U_FaninIDs,
				      generation = Generation})
    end.
...
cutlink_FromElementToElement(AgentID, From_ElementId,
			     To_ElementId) ->
    case {From_ElementId, To_ElementId} of
      {{_FromId, neuron}, {_ToId, neuron}} ->
	  cutlink_FromNeuronToNeuron(AgentID, From_ElementId,
				     To_ElementId);
      {{_FromId, sensor}, {_ToId, neuron}} ->
	  cutlink_FromSensorToNeuron(AgentID, From_ElementId,
				     To_ElementId);
      {{_FromId, neuron}, {_ToId, actuator}} ->
	  cutlink_FromNeuronToActuator(AgentID, From_ElementId,
				       To_ElementId)
    end.
...
cutlink_FromNeuronToNeuron(AgentID, From_NeuronId,
			   To_NeuronId) ->
    A = genotype:read({agent, AgentID}),
    Generation = A#agent.generation,
    %From Part
    FromN = genotype:read({neuron, From_NeuronId}),
    U_FromN = cutlink_FromNeuron(FromN, To_NeuronId,
				 Generation),
    genotype:write(U_FromN),
    %To Part
    ToN = genotype:read({neuron, To_NeuronId}),
    U_ToN = cutlink_ToNeuron(From_NeuronId, ToN,
			     Generation),
    genotype:write(U_ToN).
...
cutlink_FromNeuron(FromN, ToId, Generation) ->
    ...
    case lists:member(ToId, FromOutputIDs) of
	  ...
	  U_FromROIDs = FromROIDs --
			   [ToId],%Not necessary if not recursive...
	  FromN#neuron{output_ids = U_FromOutputIDs,
		       ro_ids = U_FromROIDs, generation = Generation};
      false ->
	  exit("ERROR:: cutlink_FromNeuron [can not "
	       "remove OID]: ~p not a member of ~p~n",
	       [ToId, FromN#neuron.id])
    end.
...
cutlink_ToNeuron(FromId, ToN, Generation) ->
    ...
    case lists:keymember(FromId, 1, ToWeightedInputs) of
      true ->
	  U_ToWeightedInputs = lists:keydelete(FromId, 1,
					       ToWeightedInputs),
	  ToN#neuron{weighted_inputs = U_ToWeightedInputs,
		     generation = Generation};
      false ->
	  exit("ERROR[can not remove IID]: ~p not a "
	       "member of ~p~n",
	       [FromId, ToN#neuron.id])
    end.

%cutlink_ToNeuron/3 cuts the connection on the ToNeuron (ToN) side. The function first checks if the FromId is a member of the ToN's weighted_inputs list, if its not, then the function exits with error. If FromId is a member, then that tuple is removed from the ToWeightedInputs list, and the updated ToN record is returned to the caller.

cutlink_FromSensorToNeuron(AgentID, From_SensorId,
			   To_NeuronId) ->
    A = genotype:read({agent, AgentID}),
    Generation = A#agent.generation,
    %From Part
    FromS = genotype:read({sensor, From_SensorId}),
    U_FromS = cutlink_FromSensor(FromS, To_NeuronId,
				 Generation),
    genotype:write(U_FromS),
    %To Part
    ToN = genotype:read({neuron, To_NeuronId}),
    U_ToN = cutlink_ToNeuron(From_SensorId, ToN,
			     Generation),
    genotype:write(U_ToN).
...
cutlink_FromSensor(FromS, ToId, Generation) ->
    ...
    case lists:member(ToId, FromFanoutIDs) of
	  ...
	  FromS#sensor{fanout_ids = U_FromFanoutIDs,
		       generation = Generation};
      false ->
	  exit("ERROR:: cutlink_FromSensor [can not "
	       "remove ToId]: ~p not a member of ~p~n",
	       [ToId, FromS#sensor.id])
    end.
...
cutlink_FromNeuronToActuator(AgentID, From_NeuronId,
			     To_ActuatorId) ->
    A = genotype:read({agent, AgentID}),
    Generation = A#agent.generation,
    %From Part
    FromN = genotype:read({neuron, From_NeuronId}),
    U_FromN = cutlink_FromNeuron(FromN, To_ActuatorId,
				 Generation),
    genotype:write(U_FromN),
    %To Part
    ToA = genotype:read({actuator, To_ActuatorId}),
    U_ToA = cutlink_ToActuator(From_NeuronId, ToA,
			       Generation),
...
cutlink_ToActuator(FromId, ToA, Generation) ->
    ...
    case lists:member(FromId, ToFaninIDs) of
	  ...
	  ToA#actuator{fanin_ids = U_ToFaninIDs,
		       generation = Generation};
      false ->
	  exit("ERROR:: cutlink_ToActuator [can not "
	       "remove FromId]: ~p not a member of ~p~n",
	       [FromId, ToA])
    end.
...
add_outlink(AgentID) ->
    ...
    Outlink_NeuronIDPool =
	filter_OutlinkIdPool(A#agent.constraint, NeuronID, NeuronIDs),
    case Outlink_NeuronIDPool -- OutputIDs of
      [] ->
	  exit("********ERROR:add_outlink:: Neuron already "
	       "connected to all ids");
      AvailableIDs ->
	  ToID = lists:nth(rand:uniform(length(AvailableIDs)),
			    AvailableIDs),
...
filter_OutlinkIdPool(C, NeuronID, NeuronIDs) ->
    case C#constraint.connection_architecture of
	  ...
	  [{{Outlink_LI, Outlink_UniqueId}, neuron}
	   || {{Outlink_LI, Outlink_UniqueId}, neuron} <- NeuronIDs,
	      Outlink_LI > LI]
    end.
...
add_inlink(AgentID) ->
    ...
    Neuron = genotype:read({neuron, NeuronID}),
    {IIDs, _WeightLists} =
	lists:unzip(N#neuron.weighted_inputs),
    Inlink_NeuronIDPool = filter_InlinkIdPool(A#agent.constraint,
					 NeuronID, NeuronIDs),
    case (SensorIDs ++ Inlink_NeuronIDPool) -- IIDs of
      [] ->
	  exit("********ERROR:add_INLink:: Neuron already "
	       "connected from all ids");
      AvailableIDs ->
	  FromID =
	      lists:nth(rand:uniform(length(AvailableIDs)),
			AvailableIDs),
...
filter_InlinkIdPool(C, NeuronID, NeuronIDs) ->
    case C#constraint.connection_architecture of
	  ...
	  [{{Inlink_LI, Inlink_UniqueId}, neuron}
	   || {{Inlink_LI, Inlink_UniqueId}, neuron} <- NeuronIDs,
	      Inlink_LI < LI]
    end.
...
add_neuron(AgentID) ->%Adds neuron and connects it to other neurons, not sensors or actuators.
    ...
    {TargetLayer, TargetNeuronIDs} =
	lists:nth(rand:uniform(length(Pattern)), Pattern),
    NewNeuronID = {{TargetLayer, genotype:generate_UniqueId()},
	       neuron},
    U_NeuronIDs = [NewNeuronID | NeuronIDs],
    U_Pattern = lists:keyreplace(TargetLayer, 1, Pattern,
				 {TargetLayer, [NewNeuronID | TargetNeuronIDs]}),
    SpecCon = A#agent.constraint,
    genotype:construct_Neuron(CortexID, Generation, SpecCon,
			      NewNeuronID, [], []),
    Inlink_NeuronIDPool = filter_InlinkIdPool(A#agent.constraint,
					 NewNeuronID, NeuronIDs),
    Outlink_NeuronIDPool =
	filter_OutlinkIdPool(A#agent.constraint, NewNeuronID,
			     NeuronIDs),
    FromElementId_Pool = Inlink_NeuronIDPool ++ SensorIDs,
    ToElementId_Pool = Outlink_NeuronIDPool,
    case (FromElementId_Pool == []) or
	   (Outlink_NeuronIDPool == [])
	of
      true ->
	  exit("********ERROR::add_neuron(AgentID)::Can't "
	       "add new neuron here, Inlink_NeuronIDPool "
	       "or Outlink_NeuronIDPool is empty.");
      false ->
	  From_ElementId =
	      lists:nth(rand:uniform(length(FromElementId_Pool)),
			FromElementId_Pool),
	  To_ElementId =
	      lists:nth(rand:uniform(length(ToElementId_Pool)),
			ToElementId_Pool),
	  link_FromElementToElement(AgentID, From_ElementId,
				    NewNeuronID),
	  link_FromElementToElement(AgentID, NewNeuronID,
				    To_ElementId),
	  U_EvoHist = [{add_neuron, From_ElementId, NewNeuronID,
			To_ElementId}
		       | A#agent.evo_hist],
	  genotype:write(Cortex#cortex{neuron_ids = U_NeuronIDs}),
	  genotype:write(A#agent{pattern = U_Pattern,
				 evo_hist = U_EvoHist})
    end.
...
outsplice(AgentID) ->
    ...
    OID =
	lists:nth(rand:uniform(length(N#neuron.output_ids)),
		  N#neuron.output_ids),
    ...
    NewNeuronID = {{NewLI, genotype:generate_UniqueId()},
	       neuron},
    SpecCon = A#agent.constraint,
    genotype:construct_Neuron(CortexID, Generation, SpecCon,
			      NewNeuronID, [], []),
    ...
    U_Pattern = case lists:keymember(NewLI, 1, Pattern) of
		  true ->
		      ...
		      lists:keyreplace(NewLI, 1, Pattern,
				       {NewLI, [NewNeuronID | InLayerIds]});
		  false -> lists:sort([{NewLI, [NewNeuronID]} | Pattern])
		end,
    ...
    %Updated agent
    EvoHist = A#agent.evo_hist,
    U_EvoHist = [{outsplice, NeuronID, NewNeuronID, OID}
		 | EvoHist],
    U_Cortex = Cortex#cortex{neuron_ids =
			 [NewNeuronID | Cortex#cortex.neuron_ids]},
...
get_NewLI(LI, LI, _Direction, _Pattern) -> LI;
get_NewLI(FromLI, ToLI, Direction, Pattern) ->
    NewLI = case Direction of
	      next -> get_NextLI(Pattern, FromLI, ToLI);
	      prev -> get_PrevLI(lists:reverse(Pattern), FromLI, ToLI)
	    end,
    NewLI.
...
get_NextLI([{FromLI, _LastLayerNeuronIDs}], FromLI,
	   ToLI) ->
    (FromLI + ToLI) / 2;
get_NextLI([{LI, _LayerNeuronIDs} | Pattern], FromLI,
	   ToLI) ->
    case LI == FromLI of
      true ->
      ...
      false -> get_NextLI(Pattern, FromLI, ToLI)
    end.
...
get_PrevLI([{FromLI, _FirstLayerNeuronIDs}], FromLI,
	   ToLI) ->
    (FromLI + ToLI) / 2;
get_PrevLI([{LI, _LayerNeuronIDs} | Pattern], FromLI,
	   ToLI) ->
    case LI == FromLI of
      true ->
      ...
      false -> get_PrevLI(Pattern, FromLI, ToLI)
    end.
...
add_sensorlink(AgentID) ->
    ...
    case NeuronIDs -- S#sensor.fanout_ids of
      [] ->
	  exit("********ERROR:add_sensorlink:: Sensor "
	       "already connected to all NeuronIDs");
      AvailableIDs ->
	  NeuronID = lists:nth(rand:uniform(length(AvailableIDs)),
			   AvailableIDs),
...
add_actuatorlink(AgentID) ->%TODO: There should be a preference towards non fully connected actuators.
    ...
    case NeuronIDs -- A#actuator.fanin_ids of
      [] ->
	  exit("********ERROR:add_actuatorlink:: Actuator "
	       "already connected from all NeuronIDs");
      AvailableIDs ->
	  NeuronID = lists:nth(rand:uniform(length(AvailableIDs)),
			   AvailableIDs),
...
add_sensor(AgentID) ->%TODO: There should be a preference towards adding sensors not yet used.
    ...
    case morphology:get_Sensors(Morphology) --
	   [(genotype:read({sensor, SID}))#sensor{id = undefined,
						   cortex_id = undefined,
						   fanout_ids = [],
	...
	of
      [] ->
	  exit("********ERROR:add_sensor(AgentID):: "
	       "NN system is already using all available "
	       "sensors");
      Available_Sensors ->
	  io:format("Available_Sensors"),
	  NewSID = {{-1, genotype:generate_UniqueId()}, sensor},
	  NewSensor =
	      (lists:nth(rand:uniform(length(Available_Sensors)),
			 Available_Sensors))#sensor{id = NewSID,
						    cortex_id = CortexID},
...
add_actuator(AgentID) ->%TODO: There should be a preference towards adding actuators not yet used.
    ...
    Cortex = genotype:read({cortex, CortexID}),
    ActuatorIDs =
	Cortex#cortex.actuator_ids,%TODO: Should we fill in all the fanin_ids locations, or just 1? and let evolution fill the rest?
    SpeCon = Agent#agent.constraint,
    Morphology = SpeCon#constraint.morphology,
    case morphology:get_Actuators(Morphology) --
	   [(genotype:read({actuator, ActuatorID}))#actuator{cortex_id =
							   undefined,
						       id = undefined,
						       fanin_ids = [],
						       generation = undefined}
	...
	of
      [] ->
	  exit("********ERROR:add_actuator(AgentID):: "
	       "NN system is already using all available "
	       "actuators");
      Available_Actuators ->
	  NewActuatorID = {{1, genotype:generate_UniqueId()}, actuator},
	  NewActuator =
	      (lists:nth(rand:uniform(length(Available_Actuators)),
			 Available_Actuators))#actuator{id = NewActuatorID,
							cortex_id = CortexID},
