...
dot_product([{IProcessID, Input} | IAcc],
	    [{IProcessID, Weights} | IWeightedInputProcessIDs], Acc) ->
    Dot = dot(Input, Weights, 0),
    dot_product(IAcc, IWeightedInputProcessIDs, Dot + Acc);
dot_product([], [{bias, [Bias]}], Acc) -> Acc + Bias;
dot_product([], [], Acc) -> Acc.
...
diff_product(IAcc, IWeightedInputProcessIDs) ->
    case get(diff_product) of
      undefined ->
	  put(diff_product, IAcc), dot_product(IAcc, IWeightedInputProcessIDs, 0);
      Prev_IAcc ->
...
input_diff([{IProcessID, Input} | IAcc],
	   [{IProcessID, Prev_Input} | Prev_IAcc], Acc) ->
    Vector_Diff = diff(Input, Prev_Input, []),
    input_diff(IAcc, Prev_IAcc,
	       [{IProcessID, Vector_Diff} | Acc]);
input_diff([], [], Acc) -> lists:reverse(Acc).
...
mult_product([{IProcessID, Input} | IAcc],
	     [{IProcessID, Weights} | IWeightedInputProcessIDs], Acc) ->
    Dot = mult(Input, Weights, 1),
    mult_product(IAcc, IWeightedInputProcessIDs, Dot * Acc);
mult_product([], [{bias, [Bias]}], Acc) -> Acc * Bias;
mult_product([], [], Acc) -> Acc.
