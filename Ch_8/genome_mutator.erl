...
mutate(AgentID)->
	...
	F = fun()->
		...
		apply_Mutators(AgentID),
		genotype:update_fingerprint(AgentID)
	end,
	mnesia:transaction(F).
%The function mutate/1 applies the available mutation operator to an agent with an id AgentID, and then adds the agent to the specie it belongs to, based on the agent's fingerprint.

	apply_Mutators(AgentID)->
		A = genotype:read({agent,AgentID}),
		Cortex = genotype:read({cortex,A#agent.cortex_id}),
		TotalNeurons = length(Cortex#cortex.neuron_ids),
		TotalMutations = rand:uniform(round(math:pow(TotalNeurons,1/2))),
...
%apply_Mutators/1 applies chooses a random number X between 1 and math:sqrt(Total_Neurons)), where Total_Neurons is the total number of neurons in the neural network, and then applies X number of randomly chosen mutation operators to the NN. The function first calculates the length of the neuron_ids list, from which it then calculates the TotalMutations value by choosing the random number between 1 and sqrt of the length of neuron_ids list. Having now chosen how many mutation operators to apply to the NN based system, the function apply_Mutators/1 calls apply_Mutators/2.
...
mutate_weights(AgentID)->
	...
	WeightedInputs = Neuron#neuron.weighted_inputs,
...
%The mutate_weights/1 function accepts the AgentID parameter, extracts the NN's cortex, and then chooses a random neuron belonging to the NN with a uniform distribution probability. Then the neuron's weighted_inputs list is extracted, and the function perturbWeightedInputs/1 is used to perturb/mutate the weights. Once the WeightedInputs have been perturbed, the agent's evolutionary history, EvoHist is updated to include the successfully applied mutate_weights mutation operator. Then the updated Agent and the updated neuron are written to the database.
	...
	perturbWeightedInputs(WeightedInputs)->
		Total_Weights=lists:sum([length(Weights) || {_InputID,Weights}<-WeightedInputs]),
		MP = 1/math:sqrt(Total_Weights),
		perturbWeightedInputs(MP,WeightedInputs,[]).
	perturbWeightedInputs(MP,[{InputID,Weights}|WeightedInputs],Acc)->
		U_Weights = perturb_weights(MP,Weights,[]),
		perturbWeightedInputs(MP,WeightedInputs,[{InputID,U_Weights}|Acc]);
...
add_bias(AgentID)->
	...
	case lists:keymember(bias, 1, WeightedInputs) of
		...
		false ->
			U_WeightedInputs = lists:append(WeightedInputs,[{bias,[rand:uniform()-0.5]}]),
			UpdatedNeuron = Neuron#neuron{
				weighted_inputs = U_WeightedInputs,
...
remove_bias(AgentID)->
	...
	case lists:keymember(bias, 1, WeightedInputs) of
		...
		true ->
			U_WeightedInputs = lists:keydelete(bias,1,WeightedInputs),
			UpdatedNeuron = Neuron#neuron{
				weighted_inputs = U_WeightedInputs,
				generation = Generation},
			EvoHist = A#agent.evo_hist,
...
mutate_af(AgentID)->
	...
	Activation_Functions = (A#agent.constraint)#constraint.neural_afs -- [ActivationFunction],
	NewActivationFunction = genotype:generate_NeuronActivationFunction(Activation_Functions),
	UpdatedNeuron = Neuron#neuron{af=NewActivationFunction,generation=Generation},
	EvoHist = A#agent.evo_hist,
	U_EvoHist = [{mutate_af,AgentID}|EvoHist],
	UpdatedActuator = A#agent{evo_hist=U_EvoHist},
	genotype:write(UpdatedNeuron),
	genotype:write(UpdatedActuator).
...
%link_FromNeuron/4 updates the record of the neuron from whom the link is being created. FromN is the record of the neuron from whom the link/connection eminates, and ToId is the id of the element to whom the link is headed towards. The function extracts the layer index of the neuron FromN, and the layer index of the element with the id ToId. Then the two layer indecies are compared, and the ToId is either added only to the FromN's output_ids list, or if the connection is recursive, ToLayerIndex =< FromLayerIndex, to output_ids and ro_ids lists. The FromN's generation is updated to the value Generation, which is the current, most recent generation, since this neuron has just been modified. Finally, the updated neuron record is then returned to the caller. On the other hand, if ToId, the id of the element to which the connection is being established, is already a member of the FromN's output_ids list, then the function exits with error.
	...
	link_ToNeuron(FromId,FromOVL,ToN,Generation)->
		...
		case lists:keymember(FromId, 1, ToWeightedInputs) of
			true ->
				exit("ERROR:add_NeuronI::[can not add IID]: ~p already a member of ~p~n",[FromId,ToN#neuron.id]);
			false ->
				U_ToWeightedInputs = [{FromId, genotype:createNeuralWeights(FromOVL,[])}|ToWeightedInputs],
				ToN#neuron{
					weighted_inputs = U_ToWeightedInputs,
					generation = Generation
				}
		end.
...
%cutlink_FromNeuron/3 cuts the connection on the FromNeuron (FromN) side. The function first checks if the ToId is a member of the output_ids list, if its not then the function exits with an error. If the ToId is a member of the output_ids list, then the function removes the ToId from the FromOutputIDs and from the FromROIDs. Even if the ToId is a recursive connection, then removing it from ro_ids updates the FromROIDs list, if its not, then no change is made to the ro_ids list. Once the lists are updated, the updated neuron record of FromN is returned to the caller.
	...
	cutlink_ToNeuron(FromId,ToN,Generation)->
		...
		case lists:keymember(FromId, 1, ToWeightedInputs) of
			true ->
				U_ToWeightedInputs = lists:keydelete(FromId,1,ToWeightedInputs),
				ToN#neuron{
					weighted_inputs = U_ToWeightedInputs,
					generation = Generation};
			false ->
...
add_inlink(AgentID)->
	...
	NeuronID = lists:nth(rand:uniform(length(NeuronIDs)),NeuronIDs),
	Neuron = genotype:read({neuron,NeuronID}),
	{IIDs,_WeightLists} = lists:unzip(N#neuron.weighted_inputs),
	case lists:append(SensorIDs,NeuronIDs) -- IIDs of
		[] ->
...
add_neuron(AgentID)->%Adds neuron and connects it to other neurons, not sensors or actuators.
	...
	From_ElementId = lists:nth(rand:uniform(length(NeuronIDs)),NeuronIDs),
	To_ElementId = lists:nth(rand:uniform(length(NeuronIDs)),NeuronIDs),
...
%The function add_neuron/1 creats a new neuron, and connects it to a randomly selected element in the NN, and form a randomly selected element in the NN. The function first reads the agent's pattern list, selects a random layer from the pattern, and then creates a new neuron id for that layer. Then, a new, unconnected neuron, is created with that neuron id. From the cortex's neuron_ids list, two random neuron ids are chosen: From_ElementId, and To_ElementId, (they can be the same ids). The function then establishes a connect from the neuron to To_ElemenId, and to the neuron from From_ElementId. Finally, the cortex's neuron_ids list is updated by appending to it the id of the newly created neuron, the agent's evo_hist is updated, and the updated cortex and agent records are written to database.
...
%Compose a feedforward Id pool, to create the splice from.
	OIDPool = case [{{TargetLayerIndex,TargetUId},TargetType} || {{TargetLayerIndex,TargetUId},TargetType} <- N#neuron.output_ids, TargetLayerIndex > LayerIndex] of
		[] ->
			exit("********ERROR:outsplice:: NeuroLink_OutputSplice OIDPool == []");
		Ids ->
			Ids
	end,
...
add_sensor(AgentID)->%TODO: There should be a preference towards adding sensors not yet used.
	...
	case morphology:get_Sensors(Morphology)--[(genotype:read({sensor,SID}))#sensor{id=undefined,cortex_id=undefined,fanout_ids=[]} || SID<-SensorIDs] of
		[] ->
...
add_actuator(AgentID)->%TODO: There should be a preference towards adding actuators not yet used.
	...
	SpeCon = Agent#agent.constraint,
	Morphology = SpeCon#constraint.morphology,
	case morphology:get_Actuators(Morphology)--[(genotype:read({actuator,ActuatorID}))#actuator{cortex_id=undefined,id=undefined,fanin_ids=[]} || ActuatorID<-ActuatorIDs] of
		[] ->
