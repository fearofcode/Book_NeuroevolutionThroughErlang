...
constructAgent(SpecieID,AgentID,SpecCon)->
	random:seed(now()),
	Generation = 0,
	{CortexID,Pattern} = constructCortex(AgentID,Generation,SpecCon),
	Agent = #agent{
		id = AgentID,
		cortex_id = CortexID,
		specie_id = SpecieID,
...
constructCortex(AgentID,Generation,SpecCon)->
	...
	Sensors = [S#sensor{id={{-1,generate_UniqueId()},sensor},cortex_id=CortexID}|| S<- morphology:get_InitSensors(Morphology)],
	Actuators = [A#actuator{id={{1,generate_UniqueId()},actuator},cortex_id=CortexID}||A<-morphology:get_InitActuators(Morphology)],
...
%The generate_NeuronActivationFunction/1 accepts a list of activation function tags, and returns a randomly chosen one. If an empty list was passed as the parameter, the function returns the standard tanh tag.
...
update_fingerprint(AgentID)->
	...
	GeneralizedSensors = [(read({sensor,SID}))#sensor{id=undefined,cortex_id=undefined} || SID<-Cortex#cortex.sensor_ids],
	GeneralizedActuators = [(read({actuator,ActuatorID}))#actuator{id=undefined,cortex_id=undefined} || ActuatorID<-Cortex#cortex.actuator_ids],
...
clone_neurons(TableName,[NeuronID|NeuronIDs])->
	...
	CloneCortexID = ets:lookup_element(TableName,Neuron#neuron.cortex_id,2),
	CloneWeightedInputs =  [{ets:lookup_element(TableName,IID,2),Weights}|| {IID,Weights} <- Neuron#neuron.weighted_inputs],
	CloneOutputIDs = [ets:lookup_element(TableName,OID,2)|| OID <- Neuron#neuron.output_ids],
	CloneROIDs =[ets:lookup_element(TableName,ROID,2)|| ROID <- Neuron#neuron.ro_ids],
	write(Neuron#neuron{
		id = CloneNeuronID,
		cortex_id = CloneCortexID,
