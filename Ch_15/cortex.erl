...
loop(Id,ExoSelfProcess,SensorProcess,{[ActuatorProcess|ActuatorProcess],MActuatorProcess},NeuronProcess,CycleAcc,FitnessAcc,EFAcc,active) ->
	receive 
		{ActuatorProcess,sync,Fitness,EndFlag} ->%io:format("Fitness:~p~n",[Fitness]),
			case Fitness == goal_reached of
				true ->
					put(goal_reached,true),
					loop(Id,ExoSelfProcess,SensorProcess,{ActuatorProcess,MActuatorProcess},NeuronProcess,CycleAcc,FitnessAcc,EFAcc+EndFlag,active);
				false ->
