...
competition(ProperlySorted_AgentSummaries,NeuralEnergyCost,PopulationLimit)->
	...
	NewGenAgentIDs = gather_survivors(AlotmentsP,Normalizer,[]),
		...
		SurvivingAgentIDs = case Normalized_MutantAlotment >= 1 of
			true ->
				MutantAgentIDs = case Normalized_MutantAlotment >= 2 of
					true ->
						[population_monitor:create_MutantAgentCopy(AgentID)|| _ <-lists:seq(1,Normalized_MutantAlotment-1)];
					false ->
						[]
				end,
...
top3(ProperlySorted_AgentSummaries,NeuralEnergyCost,PopulationLimit)->
	...
	{_TopFitnessList,_TopTotalNs,TopAgentIDs} = lists:unzip3(TopAgentSummaries),
	io:format("NeuralEnergyCost:~p~n",[NeuralEnergyCost]),
	NewGenAgentIDs = breed(Valid_AgentIds,PopulationLimit-TotalSurvivors,[]),
	{NewGenAgentIDs,TopAgentIDs}.
	...
	breed(Valid_AgentIds,OffspringIndex,Acc)->
		...
		MutantAgentID = population_monitor:create_MutantAgentCopy(Parent_AgentId),
		breed(Valid_AgentIds,OffspringIndex-1,[MutantAgentID|Acc]).
...
competition(ProperlySorted_AgentSummaries)->
	TotalEnergy = lists:sum([Fitness || {Fitness,_TotalN,_AgentID}<-ProperlySorted_AgentSummaries]),
	TotalNeurons = lists:sum([TotalN || {_Fitness,TotalN,_AgentID} <- ProperlySorted_AgentSummaries]),
	...
	Choice = rand:uniform(),
	{WinnerFitness,WinnerTotalN,WinnerAgentID}=choose_CompetitionWinner(AlotmentsP,Normalizer,Choice,0),
	{WinnerFitness,WinnerTotalN,WinnerAgentID}.
		
	choose_CompetitionWinner([{MutantAlotment,Fitness,TotalN,AgentID}|AlotmentsP],Normalizer,Choice,Range_From)->
		Range_To = Range_From+MutantAlotment/Normalizer,
		case (Choice >= Range_From) and (Choice =< Range_To) of
			true ->
				{Fitness,TotalN,AgentID};
			false ->
