...
%			io:format("Population Generation:~p Ended.~n~n~n",[U_PopGen]),
			case OpTag of
				continue ->
					...
					case (U_PopGen >= Generation_Limit) or (S#state.tot_evaluations >= Evaluation_Limit) or (BestFitness > Fitness_Goal) of
...
handle_cast({AgentID,terminated,Fitness},S) when S#state.evolutionary_algorithm == steady_state ->
	...
	case (S#state.tot_evaluations >= Evaluation_Limit) or (NewBestFitness > Fitness_Goal) of
		true ->
...
handle_cast({From,evaluations,SpecieID,AgentEvalAcc,AgentCycleAcc,AgentTimeAcc},S)->
		...
		_ ->
			...
			case S#state.benchmarker_pid of
				undefined ->
					ok;
				ProcessID ->
					ProcessID ! {S#state.population_id,completed,T}
			end
	end.
...
gather_STATS(PopulationID,EvaluationsAcc)->
	...
	F = fun() ->
		P = genotype:read({population,PopulationID}),
		T = P#population.trace,
		SpecieSTATS = [update_SpecieSTAT(SpecieID,TimeStamp) || SpecieID<-P#population.specie_ids],
